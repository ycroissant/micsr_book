...
\index[general]{probit|)}\index[general]{logit|)}
\idxfun{mode\_choice}{micsr.data}
...
\idxfun{binomreg}{micsr}\idxfun{update}{base}\idxfun{gaze}{micsr}
...
\idxfun{filter}{stats}\idxfun{ifelse}{base}
...
Pearson statistic $X ^ 2$ given by @eq-pearson_statistic.\index[general]{residuals!Pearson's}
...
and they are related to the latent variable $y^*$ used in @sec-latent_variable\index[general]{latent variable}. Remind that $y_n ^ * = \mu_n + \epsilon^*_n$, with $\mu_n = \alpha + \beta ^ \top x_n$ and $\epsilon_n \sim \mathcal{N}(0, \sigma)$. Then, considering the latent variable, the residual can be defined as $\hat{\epsilon} ^ * _n = y_n ^ * - \hat{\mu}_n$. This residual can't be computed because $y_n ^ *$ is unobserved, we only observe $y_n = 1$ if $y_n ^ * > 0$ and $y_n = 0$ if $y_n ^ * \leq 0$. However, its expectation can be computed. For $y_n = 1$:
