...
can justify the use of these models. @sec-glm presents the generalized linear models from which the binomial model is a special case. @sec-binom_evaluation is devoted to estimation, evaluation and testing of binomial models. Finally, @sec-endog_probit presents relevant estimators when some covariates are endogenous.
...
concern the transport mode chosen for work trips by a sample of 842 individuals in
...
@EDEL:LUCA:SVIR:17. The aim of their study is to analyze the presence
...
- they have an intuitive meaning are they are the difference between
  ...
  estimates the conditional mean of the response,
...
The `stats::glm` function use an iterative weighted least
...
is rather complicated and depends on $y$. However, its expectation (@eq-infobinom) can be be expressed compactly in terms of the inverse mills ratio, defined by:
 $\r(z) = \phi(z) / \Phi(z)$. Noting that $\phi(z) / (1 - \Phi(z)) = \phi(-z) / \Phi(-z) = \mu(-z)$ by symmetry of the normal distribution, @eq-infobinom simplifies to:
...
and they are related to the latent variable $y^*$ used in @sec-latent_variable. Remind that $y_n ^ * = \mu_n + \epsilon^*_n$, with $\mu_n = \alpha + \beta ^ \top x_n$ and $\epsilon_n \sim \mathcal{N}(0, \sigma)$. Then, considering the latent variable, the residual can be defined as $\hat{\epsilon} ^ * _n = y_n ^ * - \hat{\mu}_n$. This residual can't be computed because $y_n ^ *$ is unobserved, we only observe $y_n = 1$ if $y_n ^ * > 0$ and $y_n = 0$ if $y_n ^ * \leq 0$. However, its expectation can be computed. For $y_n = 1$:
...
The constrained model implies the two following hypothesis: $\beta_o = \beta_i =
...
Tests can be computed using several functions in the **lmtest** package [@ZEIL:HOTH:02].
...
car::linearHypothesis(pbt_unconst, 
                      c("ivtime = 8 * cost", "ovtime = 8 * cost")) %>% gaze
```
...
#### Conditional moment tests {#sec-cond_moment_binomial}
...
For the probit model, the first four moments of the truncated normal distribution should be computed. A recursive formula for the moments of a normal variable $x \sim \mathcal(\eta, \sigma)$ with $l\leq x \leq u$ is:^[Unpublished note by Eric Orjebin, 2014, founded on the wikipedia page entitled "Truncated normal distribution".]
...
For a residual of the probit model, we have $\eta = 0$ and $\sigma = 1$ and the truncature is $-\mu_n$. We then obtain, for $y_n^* \leq 0$ ($l = - \infty$ and $u = -\mu_n$) and $y_n ^ * > 0$ ($l = - \mu_n$ and $u = +\infty$) is:
...
1 & \mbox{omit. variable} & \psi_n & \mbox{E}(\epsilon_n w_n\mid x_n) = 0 & \frac{1}{N}\sum_n \psi_n w_n\\
...
We consider here the case where $y_n ^ *$ is not observed, but only its sign. Therefore, we observe $y_n$ equal to 0 or 1, or $q_n = 2 y_n - 1$ equal to -1 or +1. The log-likelihood is then $\ln L = \sum_{n=1} ^ N \ln g(w_n) + \ln f(y_n \mid w_n)$, where $g(w)$ is given by @eq-marg_density_w and:
...
Denoting $\Pi ^ * = C \Pi$, $w_n^* = C w_n$ and $\rho ^ * = C
\sigma_{\epsilon\nu}$ we have : $\theta ^ \top u_n = \gamma ^ \top
z_n + \rho ^ {* \top} (w_n^* - \Pi ^ *)$ and $\sigma ^ 2 =
...
\frac{1}{2} (w_n ^ * - \Pi ^ * v_n) ^ \top (w_n ^ * - \Pi ^ * v_n) \\
\ln f(y_n \mid w_n) &=& \Phi\left((2 y_n - 1) \frac{\gamma ^ \top z_n + \rho ^ {*
\top} (w_n^* - \Pi ^ * v_n)}
...
\mid w_n)$ with respect to $\gamma$, $\rho ^*$, $\Pi ^ *$ and $C$. $\sigma_\epsilon$ is not identified and can be set to 1.
...
$\hat{\Pi}$ obtained by maximizing $\sum_{n=1} ^ N \ln g(w_n)$. This is a
...
w_n - \hat{\Pi} v_n$ and, in the second step, $\hat{\gamma}$ and $\hat{\rho}$
...
This two-steps instrumental variable estimator and test has been extended for
...
$$
@SMIT:BLUN:86 proposed an endogeneity test based on the two-steps estimator. As for the linear instrumental variable estimator, it is a Wald test that $\rho = 0$, but it uses the simple probit estimation of the covariance matrix.

### Minimum $\chi ^ 2$ estimator

@NEWE:87 argued that @AMEM:78's minimum
chi-square estimator can be used in this context and is more efficient than the two-steps estimator. This estimator is computed in 5 steps:
...
1. using a probit regress $y$ on the whole set of exogenous
  variables $v_n ^ \top = (1, x_{n1} ^ \top, x_{n2} ^ \top)$ and on the previously computed residuals
...
1. using a probit, regress $y$ on 
  $z_n^\top = (1, x_1^\top, \hat{\nu}_n ^ \top)$ and on the fitted values $\hat{w}_{gn}$ computed on the first
  step; save the coefficients of the fitted values $\hat{\delta}$ and
  compute $\hat{\rho} = \hat{\lambda} - \hat{\delta}$,
1. regress $\hat{\rho} ^ \top w_n$ on the whole set of exogenous
  variables $v_n$, save the covariance matrix $\hat{\Sigma}_{2}$ and
  compute $\hat{\Omega} = \hat{\Sigma}_1 + \hat{\Sigma}_2$,
1. compute the minimum the minimum $\chi^2$ estimator $\hat{\beta}$ and
...
@ADKI:CART:SIMP:07 and @ADKI:12 analyzed the effect of managerial incentives on the use of foreign-exchange derivatives for hedging by U.S. bank holding companies, for the 1996-2000 period. The dependent variable `federiv` is 1 if the bank uses foreign-exchange derivatives.
...
The coefficients of `linstown`, `bonus` and `optval` have the expected sign. `linsown` has a positive sign, which must be driven by the strength of the regulatory constraints. Note that the standard deviations are much smaller for the highly parametrized ML estimator, compared to the two-steps and the minimum $\chi^2$ estimator.

To perform the endogeneity test, we first estimate the two-steps estimator with the probit estimator of the covariance matrix of the coefficients. This is performed by setting the supplementary argument `robust` to `FALSE`:
...
0 to 18. Denote $I_{nt}$ the value of this variable for country $n$ on year $t$ divided by 18, so that $I_{nt}$ equal 0 and 1 indicates no and perfect financial liberalization. It is assumed that the yearly variation of the index is given by the following equation, denoting $I^*_{nt}$ the desired value of $I_{nt}$:
...
The authors tested the possibility of regional diffusion: countries within a region would then be induced to catch up with the highest level of liberalization observed within the region. Therefore, they introduce a covariate which is equal
