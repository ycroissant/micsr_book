...
@sec-poisson_model is devoted to the benchmark count model, the Poisson model. @sec-overdisp_zero discusses two common features of count data: over-dispersion and excess of zero. Finally @sec-endog_count deals with the endogeneity problem with a count response. 
...
and the United States. `health_ins` [@DEB:TRIV:02]\idxdata{health\_ins}{micsr.count} focus on the link between health insurance
and doctor visits using a randomized experiment in the United States,
as `health_reform` [@WINK:04]\idxdata{health\_reform}{micsr.count} analyzes the effect of an health insurance reform in
Germany. `elderly` [@DEB:TRIV:97]\idxdata{elderly}{micsr.count}  focus on the demand for health care by elderly in
the US and `hospitalization` [@GEIL:MILL:ROTT:ZIMM:97]\idxdata{hospitalization}{micsr.count} uses as the response the number of stays at
the hospital.

The response of `amc12` [@ELLI:SWAN:16]\idxdata{amc12}{micsr.count} is the number of students per school who got a very high
score at the AMC12 test (a test on mathematics). `asymptotic`
[@KOEN:ZEIL:09]\idxdata{asymtotic}{micsr.count} is a
...
when new leniency program was introduced. `cigmales` [@MULL:97]\idxdata{cigmales}{micsr.count} measures the
...
individuals who got a Ph. D. in bio-chemical. `somerville`
...
`cigmales`\idxdata{cigmales}{micsr.count} (the number of cigarettes smoked daily).
...
- the second one is the problem of **excess of zero**, which means that
...
strictly positive values. An example is `cigmales`\idxdata{cigmales}{micsr.count}, where 0 means no
...
ratio between the empirical share of zero and the Poisson probability of 0 and, unsurprisingly, this ratio is huge for `cigmales`\idxdata{cigmales}{micsr.count}.
...
present here the test advocated by @CAME:TRIV:86\index[author]{Cameron}\index[author]{Trivedi} who use an
...
largely remains, which suggests that a model that take the specificity
...
The **zero-inflated Poisson** (or **ZIP** model),^[also called with zeros model.] proposed by @LAMB:92\index[author]{Lambert} starts from a count model, for example a Poisson with a set of covariates denoted $z_2$ and corresponding coefficients $\gamma_2$. The probability of 0 ($e^{-\gamma_2^\top z_{n2}}$) is inflated using a further parameter $\psi_n$ between 0 and 1:
...
\idxfun{AIC}{stats}\index[general]{Aikaike information criteria|(}
...
The performance of the hurdle model is slightly better than ZIP model. The AIC is much less than the one of the Poisson model, but much larger than the one of the Negbin2 model.
...
Finally the Negbin2 hurdle or zip model can be compared either to the Negbin2 Poisson model (for which the treatment of excess of 0 is added) or to the hurdle or zip Poisson model (for which the treatment of over-dispersion is added):
\idxfun{AIC}{stats}
...
figure
```
\index[general]{Aikaike information criteria|)}
\idxdata[)]{trips}{micsr}
