...
`amc12` [@ELLI:SWAN:16]\idxdata{amc12}{micsr.count} uses the number of students per school who got a very high
...
`asymptotic`\idxdata{asymptotic}{micsr.count} (the number of covariates in an econometric equation),
...
ratio between the empirical share of zero and the Poisson probability of 0 and, unsurprisingly, this ratio is huge for `cigmales`\idxdata{cigmales}{micsr.count}. Some models have been proposed that have a special
...
\idxfun{rootogram}{countreg}\idxfun{autoplot}{countreg}\idxfun{coord\_cartesian}{ggplot2}\idxfun{ggarange}{ggpubr}
...
model, proposed by @LAMB:92\index[author]{Lambert} starts from a count model, for example a Poisson with a set of covariates denoted $z_2$ and corresponding coefficients $\gamma_2$. The probability of 0 ($e^{-\gamma_2^\top
...
\idxdata[(]{trips}{micsr}
\idxfun{hurdle}{countreg}\idxfun{update}{stats}
...
\idxfun{coord\_cartesian}{ggplot2}\idxfun{ggarange}{ggpubr}
...
$\hat{S} = \sum_{n = 1} ^ N \hat{\epsilon}_n ^ 2 w_n w_n ^ \top$  matrix can be constructed and used in a second step to get the more efficient GMM estimator. \index[general]{general method of moments estimator!Poisson model}
...
In two seminal papers, @HECK:76\index[author]{Heckman} and @HECK:79\index[author]{Heckman} considered the case where
...
argument, which can be either `"twosteps"` for the two-step non-linear
