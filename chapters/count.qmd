...
care. `doctor_aus`[@CAME:TRIV:86], `doctor_cal` [@GURM:97], `doctor_ger`
[@SANT:WIND:01] and `doctor_us` [@MULL:98] concern
annual doctor visits, respectively in Australia, California, Germany
and the United States. `health_ins` [@DEB:TRIV:02] focus on the link between health insurance
and doctor visits using a randomized experiment in the United States,
as `health_reform`[@WINK:04] analyzes the effect of an health insurance reform in
Germany. `elderly` [@DEB:TRIV:97]  focus on the demand for health care by elderly in
the US and `hospitalization` [@GEIL:MILL:ROTT:ZIMM:97] uses as the response the number of stays at
the hospital.

`amc12` [@ELLI:SWAN:16] uses the number of students per school who got a very high
...
observations increases. `bids` [@CAME:JOHA:97] analyzes the number of bids received by
126 firms that were targets of tender offers. `cartels` [@MILL:09] is a time
...
when new leniency program was introduced. `cigmales` [@MULL:97] measures the
number of cigarettes daily smoked by males in the United
States. `majordrg` [@GREE:01] contain data about the number of major derogatory
...
[@SELL:STOL:CHAV:85] reports the
number of recreational visits to a lake in Texas. `strikes` [@KENN:85] is a time
series that reports the number of strikes (and also their length) in
the United States.  `trips` [@TERZ:98] contains the number of trips taken by
