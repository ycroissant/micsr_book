...
Consider the `retirement` data set [@AN:CHRI:GUPT:04]\index[author]{An}\index[author]{Christensen}\index[author]{Gupta}. It contains the join
...
Consider the `recall` data set, initially used by @KATZ:86\index[author]{Katz} and latter by @SUEY:95\index[author]{Sueyoshi}. It contains 1045 spells of unemployment in 1980.

```{r}
recall %>% head(3)
```

The analysis of @KATZ:86\index[author]{Katz} showed that an important transition form unemployment was a recall from the previous employer. The `end` covariate indicates the different modalities of transition:
...
table 4 p. 484]\index[author]{Werner}. The results are presented on table @tbl-intkakadu.
