...
from the biostatistics (lifetime after an hearth transplant for example) and operations research (lifetime of a light bulb) literature. These data are called **duration** or **transition** data. The latter term indicates that we are interested by the time span of the transition from one state to another, the latter being called **death** and **failure**, respectively in the biostatistics and the operations research fields. For a given time, the observations for which the transition didn't take place are called **at-risk** observations. 

Duration data have two main characteristics, that require the use of
specific models. The first one is that they are continuous positive variables. 
The second one is that they are often censored, especially right-censored; for example, trying to modelize the length of unemployment spells, we'll use survey data on employment realized on a given period (for example two years). At the end of the survey, some individuals are still unemployed because the spell is not finished, so that the duration of their spell is not observed, but only the fact that it is greater than the observed duration at the end of the survey.

Denote $t$ the duration response. Its distribution is given by a
...
$S(t) = P(T > t) = 1 - F(t)$.^[The survival function is sometimes defined as
    $S(t) = P(T \geq t)$, see @CAME:TRIV:05.] For example, if $T$ is measured in
...
times, denoted $t_1, t_2, \ldots t_k, \ldots t_K$. If there are no ties,
...
## The Kaplan-Meier non-parametric estimator {#sec-kaplan_maier}
...
indicate the number of months between the discovery of an oil field and
...
spell is greater than one month is $98.1$%. For $t_2= 3$, we have
...
for each time where some spells end. The object
...
There is a `plot` method for `survfit` objects that draw the survival curve and its confidence interval.
...
-   `event` : a dummy for observed duration (ie uncensored
...
We now use the `unemp_teachers` data set, initially used by @KAST:VAND:22. It contains 3064 observations of unemployed dutch teachers. Some of them performed a training, they are identified by a dummy called `training`. We restrict the sample to those who didn't performed a training:
...
The **survminer** package provides the **ggsurvplot** function, based on **ggplot2**, which enables to draw nice and highly customizable survival curves, as on @fig-survival_curve where we set the `risk.table` argument to `TRUE` in order to have a table of observations at risk just below the figure.
...
In this section, we introduce covariates that may explain the length of the duration, or the value of the hazard function. Two kind of models can be developed. The full parametric model specify the whole distribution of the hazard function. On the contrary, in a semi parametric approach, the hazard is the product of two component, the first one depending on time and some parameters and the second one depending only on the covariates and some other parameters. Then, only the second component is estimated and therefore, the shape of the hazard doesn't need to be specified. We'll first consider the benchmark exponential model, for which the hazard is constant.^[This section relies heavily on @LANC:90., pp.17-20.]
...
$\mbox{V}(T) = \frac{1}{\theta ^ 2}$. Therefore, for the exponential distribution, the mean equals the standard deviation. Moreover, the unit of measurement of $\theta$ is the inverse of the time unit, so that $\theta dt$ is a number without unit.
...
hazard, $Z$ is simply proportional to $T$, as $dZ = \theta dT$ and the cumulative hazard is then $\theta T$. For example, if the constant hazard is 0.1 and the time is measured in days, a variation of 1 in the initial time scale (one day) corresponds to a variation of 0.1 on the transformed time scale.
...
$\ln \Gamma$. $\psi$ is called the digamma function and $\psi(1)=-\gamma$, $\gamma \approx 0.57721$ being the Euler-Mascheronni constant. $\psi'$ is called the trigamma function and
...
$$
which is the extreme value distribution of type 1, for which $F(U) = 1 - e^{-e^U}$, $S(U) = e^{-e^U}$, $\mbox{E}(U) = - \gamma$ and $\mbox{V}(U) = \pi ^ 2 / 6$. As $U = \ln Z = \ln T + \ln \theta$, we can write:
...
The `oil` data set doesn't have censored observations. Therefore, OLS estimation is consistent, even if it is less efficient that maximum likelihood if the distribution is really exponential. We estimated a model by maximum likelihood in @sec-ml_est_expon, with `p98` and `varp98` as covariates. We reproduce this estimation using `surveg` and we compare the results with those obtained using `lm`. The `survreg` function has a `dist` argument that we set to `"exponential"`. The response should be generated using the `Surv` function, with the duration as only argument in this example as all the observations are uncensored:
...
We then estimate the model for unemployment duration of Dutch teachers using a covariates the age, the gender and (in log), the last hourly wage:
...
The results indicate that male's unemployment spell duration are `r round(coef(un_exp)["gendermale"] * 100)`% longer than females'. One more year of age increases the lenght of the spell by `r round(coef(un_exp)["age"] * 100, 2)`%. Finally, 1% of more previous wage increases the duration of the spell by `r round(coef(un_exp)["log(wage)"], 2)`%.


### Accelerated failure time model

The previous model is a special case of a class of models called
**accelerated failure time** (**AFT** in short), for which the duration can be written
as: $T_n = Z / \lambda_n$, $\lambda_n = e ^ {\gamma ^ \top z_n}$ and $Z$ is a random
variable that don't depend on $\gamma$ and $x_n$. $\lambda_n$
...
equivalently $U = \ln Z$ is extreme value of type1. Therefore, the AFT models are based on the following specification:

$$
\ln T_n = - \ln \lambda_n + U_n
$$
where $\lambda_n$ can be parametrized as $e^{-\gamma ^ \top z_n}$, so that $\ln T_n = \gamma ^ \top z_n + U_n$. Different choices of the distribution of $U_n$ leads to different flavors of AFT models. The basic exponential model is an AFT model with $U$ following an extreme value type-1 distribution. Two natural extensions of this model consist on  introducing a supplementary parameter $\sigma = 1 / \delta$ or considering different distributions for $U$:
...
This is a Weibull distribution, with a shape parameter equal to $\delta$
...
$$
\theta(t) = \frac{\delta}{t} e^ {\delta \epsilon} \;\mbox{and} \; S(t) = e ^ {- e ^ {\delta \epsilon}}
$$
...
With $U \sim N(0, 1)$,
$\ln T = - \ln \lambda + \sigma U \sim N(-\ln \lambda, \sigma)$ and therefore
...
$$
\theta(t) = \frac{\delta}{t}\frac{\phi(\delta \epsilon)}{1 - \Phi(\delta \epsilon)} 
\; \mbox{and}\; S(t) = 1 - \Phi(\delta\epsilon)
$$

$$
\ln L = \sum_{n=1} ^ n d_n \left[\ln \delta - \ln t + \ln \phi(\delta\epsilon_n) \right] + (1 - d) \ln (1 - \Phi(\delta\epsilon))
$$
...
If $\delta = 1$, the hazard reduce to $\theta(t) = v / (1 + vT)$ and is
...
Finally, if $\delta > 1$, the hazard function is U-shaped, with a
...
We first plot the hazard curves for a specific individual: we choose a white married homeowner male aged 31 with 7 years of education, 2 dependents, with unemployment insurance, in a county with a 7% unemployment rate, with a working wife and the reference levels for `industry` and `occupation`. We then compute $\hat{\lambda}$ for the different models:
...
c(s_llog, s_lnorm, s_wei)
```

$\delta$ being larger than one for the log-logistic, we have a U-shaped distribution for the hazard function. For the Weibull model, $\delta < 1$, the hazard curve is then decreasing. 
...
The results of the 4 AFT models are presented in @tbl-acc_failure.^[To save place, we've omitted the `industry` and `occupation` covariates.]
...
#| tbl-cap: "Accelerated failure model for unemployment duration"
modelsummary::msummary(list(exponential = un_exp, weibull = un_wei, 
                            `log-logistic` = un_llog, `log-normal` = un_lnorm),
                       coef_omit = "(occupation|industry)")
```

### Proportional hazard models

The proportional hazard models (**PH** in short) start with the specification of the hazard function. More specifically, it is assumed that the hazard function can be written as the product of two terms:
...
-   the second one depends on some covariates and some unknown
    parameters, but not on duration, it is most of the time parametrized as $e ^ {\gamma ^ \top z} = e ^ {\alpha + \beta ^ \top x}$.
...
$$
\theta(t_n) = e^{\gamma ^ \top z_n} \delta t_n ^ {\delta - 1} = \frac{\delta}{t}e^{\delta \ln t_n + \gamma ^ \top z_n}\;\mbox{and}\; S(t_n) = e ^ {- e ^ {- \gamma^\top z_n} t_n ^ \delta} = e ^ {- e ^ {\delta \ln t_n + \gamma^\top z_n}}
$$
The density is the product of these two functions and it corresponds to a Weibull distribution with a scale parameter equal to $e^{-\gamma ^ \top z_n / \delta}$ and a shape parameter equal to $\delta$. 
...
functional form of $\lambda(t, \rho)$ should be specified and the full
set of parameters $(\rho, \gamma)$ are jointly estimated. Two natural
specifications for the baseline hazard function are $\lambda(t) = c$ and
$\lambda(t) = c\delta t ^ {\delta - 1}$, which leads respectively to the exponential
and the Weibull proportional hazard models. These two
models are exactly the same as their accelerated failure counterpart,
except that the parametrization is different: starting from the PH coefficients, the AFT coefficients are $- \gamma / \delta$. If $\delta = 1$ (the exponential model) the two vectors of coefficients are just opposite. 
...
The Cox proportional hazard model [@COX:72; @COX:75] is semi-parametric as only the $f()$
...
observations for which $t_m \geq t_n$, ie the set of observations at
risk at time $t_n$:
...
$$\ln L = \sum_{n=1}^N\delta_n\left(\gamma ^ \top z_n - \ln \sum_{n \in R_n}e^{\gamma ^ \top z_n}\right)$$
...
the event occured first for $n=2$, the probability for observation $2$
and $3$ is:
...
Consider a population with two groups characterized by two different constant hazard rates, 0.4 for the first group and 0.1 for the second group.^[This example is from @CAME:TRIV:05, p.611.] If there are 100 individuals in both groups, the number of events (observations at risk) for the first group is $0.4 \times 100 = 40$ (60) for the first period, $0.4 \times 60 = 24$ (36) for the second one and $0.4 \times 36 = 21.6$ (12.96) for the third one. For the second group, we get $0.1 \times 100 = 10$ (90) for the first period, $0.1 \times 90 = 9$ (81), $0.1 \times 81 = 8.1$ (72.9) and $0.1 \times 72.9 = 7.29$ (65.61). Summing for the two groups, we get 50 (150), 33 (117), 22.5 (94.5). The hazard is then $50 / 200 = 0.25$ for the first period, $33/150 = 0.22$ for the second one and $22.5 / 117 = 0.1923$ for the third period. The hazard rate for the whole population is therefore decreasing, although it is constant for every individual. This result is easy to understand as, as times going, the proportion of individuals with the low hazard rate in the observations at risk is increasing.
It is therefore important to detect the presence of heterogeneity and, in presence of heterogeneity, to fit models that take heterogeneity into account.

### Detecting heterogeneity

If heterogeneity is present, fitting a model that doesn't take it into account results in a wrong specification that can be tested using **generalized residuals**. In the context of duration models, integrated hazard can be considered as generalized residuals, as, if the specification is correct, their distribution is a unit exponential. Using the accelerating failure specification, remind that the integrated hazard is $\epsilon_n = \Lambda(t_n) = - \ln S(t_n) = e ^ {\delta (\ln t - \gamma ^ \top z_n)}$. The distribution of $\epsilon_n$ doesn't depend on $x_n$. Moreover, being a unit exponential distribution, its first 4 uncentered moments are $1, 2, 6, 24$. To get an idea of the relevance of the specification, we can plot minus the log of the empirical survival (which is the number of observations $m$ such that $\epsilon_m > \epsilon_n$ divided by the total number of observations) against the generalized residuals; the points should then be grossly aligned on the 45^o^ line. For censored observations, the duration is not known and the integrated hazard is replaced by its expectation. Denoting $L$ the censoring time, the expectation is:
...
```{r}
#| tbl-cap: "Generalized residuals for the Weibull model"
...
@fig-gres_weibull plots the generalized residuals. The fit seems excellent for low values of the generalized residuals, and bad for values higher than 4 (about 2 percent of the sample). The empirical moments are:
...
### The Weibull-Gamma model

To take into account the heterogeneity, we can use mixing distributions, either finite (taking for example 2 categories of individuals like in the introductory example) or infinite. We'll present in this section a model very widely used for its simplicity, the Weibull-Gamma model. Assume that the survival function for individual $n$ is: 

$$
S(t_n\mid x_n,\nu_n) = e ^ {-\mu_n\nu_n t_n ^ \delta}
$$ {#eq-cond_surv_weibull}

where as usual, $\mu_n = e ^ {\alpha + \beta x_n}$. $\nu_n$ is an unobserved term that is specific to the individual. If it were observed, we would get the usual Weibull model. As it is unobserved, we have to consider the unconditional survival function, ie we have to integrate out @eq-cond_surv_weibull with $\nu$. Denoting $f$ the density of $\nu$, we get:
...
as $\chi \rightarrow 0$, $\ln \left(1 + \chi \mu_n t_n ^ \delta\right)\rightarrow \chi\mu_n t_n ^ \delta$ so that the Weibull is obtained as the limiting case where the variance of the mixing distribution tends to 0. Remind from @tbl-acc_failure that, for the weibull AFT model, the scale parameter was $\sigma = `r round(un_wei$scale, 2)`$, which corresponds to $\delta = 1 / `r round(un_wei$scale, 2)` = `r round(1/ un_wei$scale, 2)`$ and therefore to a decreasing hazard rate. The `micsr::weibreg` function estimates the Weibull-Gamma model when the `mixing` argument to `TRUE`:
...
The supplementary parameter ($\chi$) is called `"var"` and is highly significantly different from 0, so that the hypothesis of homogeneity is highly rejected. Note also that, with the Weibull-Gamma model, the shape parameter is now significantly greater than 1, so that the hazard rate is increasing.
...
Consider the `recall` data set, initially used by @KATZ:86 and latter by @SUEY:95. It contains 1045 spells of unemployment in 1980.

```{r}
recall %>% head(3)
```

The analysis of @KATZ:86 showed that an important transition form unemployment was a recall from the previous employer. The `end` covariate indicates the different modalities of transition:

```{r}
recall %>% count(end)
```

We consider the `new-job` and `recall` modalities as an indication of a complete spell. The model for unemployment spells is estimated using as covariates the age, the sex, years of education, the number of dependents (`nb`), a dummy for unemployment insurance during the spell (`ui`), marital status, county unemployment rate (`unemp`), wife's employment status, a dummy for home ownership and factors for occupation and industry. The competing risk model can then be estimated using the following two calls to `coxph`:
...
$$
The probability of a transition in the $a$^th^ interval is the product of the discrete hazard for interval $a$ and the value of the survival function at the beginning of this interval (ie $t_{a-1}$). For observation $n$, the contribution to the likelihood is then:
...
This is an unbalanced pooled binomial model, the number of observation for each individual being the index of the interval of its transition. @eq-binomial_duration deals correctly for right-censored observation as, in this case, $y_{na_n} = 0$ and the correct term ($1 - F_{na_n}$) is introduced in the log-likelihood. 
...
are respectively denoted $\mu_l$ and $\mu_u$. For the first sub-figure,
we have $\mu_l = 0$, so that the interval is left-censored. On the
second sub-figure, both the lower and the upper bound are
observed. Finally, on the last sub-figure, only the lower bound is
...
Weibul distribution. For the first one, a simple parametrization is to set
...
@WERN:99. The study concerns the Kakadu Conservation zone, a part of the Kakadu National park and the survey aimed to measure the willingness to pay to preserve this zone from mining. The double-bounded,
discrete-choice elicitation method of @HANE:91 is used; a respondent
