...
We then estimate the model for unemployment duration of Dutch teachers using as covariates the age, the gender and (in log) the last hourly wage:
...
The results of the 4 AFT models are presented in @tbl-acc_failure.
...
The **proportional hazard** models (**PH** in short) starts with the specification of the hazard function. More specifically, it is assumed that the hazard function can be written as the product of two terms:
...
\index[general]{generalized residuals!Weibull model|)}
@fig-gres_weibull plots the generalized residuals, that has been computed using the `micsr::gres` function\idxfun{gres}{micsr. The fit seems excellent for low values of the generalized residuals, and bad for values higher than 4 (about 2 percent of the sample)
...
Weibull distribution. For the first one, a simple parametrization is to set
