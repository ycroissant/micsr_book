...
\idxfun{mutate}{dplyr}\idxfun{ifelse}{base}\idxfun{count}{dplyr}\idxfun{pivot\_wider}{dplyr}\idxfun{rename}{dplyr}
...
\idxfun{filter}{dplyr}\idxfun{select}{dplyr}\idxfun{mutate}{dplyr}\idxfun{head}{base}
...
\index[general]{Kaplan-Meier estimator!different groups|)}
\idxdata[)]{unemp\_teacher}{micsr.data}
\index[general]{Kaplan-Meier estimator|)}
...
$$
where $\lambda_n$ can be parametrized as $e^{-\gamma ^ \top z_n}$, so that $\ln T_n = \gamma ^ \top z_n + U_n$. Different choices of the distribution of $U_n$ leads to different flavors of AFT models. The basic exponential model is an AFT model with $U$ following an extreme value type-1 distribution. 
\index[general]{accelerated failure time model!exponential model|)}
\index[general]{accelerated failure time model!weibul model|(}
Two natural extensions of this model consist on  introducing a supplementary parameter $\sigma = 1 / \delta$ or considering different distributions for $U$:
...
\idxfun{ggplot}{ggplot2}\idxfun{dnorm}{stats}\idxfun{pnorm}{stats}\idxfun{geom\_functions}{ggplot2}\idxfun{aes}{ggplot2}
...
\idxfun{coxph}{suvival}\idxfun{Surv}{survival}
...
\idxfun{update}{base}\idxfun{factor}{base}\idxfun{glm}{stats}\idxfun{binomial}{stats}
