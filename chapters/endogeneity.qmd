...
observation in the entity. A special case is when the same individuals
(in a wide sense, it can be households, firms, countries, ...) are
...
of this missing covariates is in the error term and the correlation
with the observed covariate induces a correlation between $x$ and
$\epsilon$ ($\mbox{E}(\epsilon \mid x) \neq 0$). Consider now the case
where $\mbox{E}(\eta \mid x) \neq 0$ but $\mbox{E}(\nu \mid x) = 0$. In
...
estimate the return to education, using a Mincer earning function. In the following
