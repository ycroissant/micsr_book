...
dinc_elig <- vietnam %>%
    ...
    mutate(dmean = mean_yes - mean_no, dsd = sqrt(sd_no ^ 2 + sd_yes ^ 2)) %>%
...
dshare_elig <- veterans %>%
    pivot_wider(names_from = eligible, values_from = c(mean, sd)) %>%
    mutate(dshare = mean_yes - mean_no, sd_dshare = sqrt(sd_yes ^ 2 + sd_no ^ 2)) %>%
...
wald %>%
    ...
    geom_ribbon(aes(ymin = wald - 1.96 * sd, ymax = wald + 1.96 * sd), fill = "lightgrey") +
    geom_hline(yintercept = 0, linetype = "dashed") +
...
slaves_first <- lm(log(slarea) ~ atlantic + indian +
                       redsea + sahara, sltd)
modelsummary::msummary(slaves_first, estimate = "{estimate} ({std.error})",
                       statistic = NULL,
...
modelsummary::msummary(slaves_second, estimate = "{estimate} ({std.error})",
                       statistic = NULL,
                       coef_omit = "(colony)", output = "markdown",
                       gof_map = c("nobs", "r.squared", "F"))
```
...
valids. This hypothesis is not rejected, even at the 10% level.
