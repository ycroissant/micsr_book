...
\idxfun{length}{base}\idxfun{sum}{base}\idxfun{lfactorial}{base}
...
\idxfun{summarise}{dplyr}\idxfun{mean}{base}\idxfun{sd}{stats}
...
\idxfun{nrow}{base}\idxfun{mean}{base}
...
\idxfun{mean}{base}\idxfun{length}{base}\idxfun{sqrt}{base}\idxfun{round}{base}
...
Starting from an initial vector of parameters $\beta_i$, we use a first
order Taylor expansion of the gradient for $\beta_{i+1}$ "close to"
$\beta_i$:

$$
g(\beta_{i+1}) \approx g(\beta_i) + H(\beta_i) (\beta_{i+1} - \beta_i)
$$

Solving the first order conditions for a maximum, we
should have $g(\beta_i) + H(\beta_i) (\beta_{i+1} - \beta_i) = 0$,
which leads to:

$$
\beta_{i+1} = \beta_i - H(\beta_i) ^ {-1} g(\beta_i)
$$ {\#eq-updating_rule}

Except if the gradient is a linear function of $\beta$, $\beta_{i+1}$ is
not the maximum, but it is closer to the maximum than $\beta_i$ and
...
\idxfun{function}{base}\idxfun{exp}{base}\idxfun{sqrt}{base}\idxfun{crossprod}{base}\idxfun{apply}{base}\idxfun{drop}{base}
...
We can see that we obtained an updated vector of coefficients
...
$10^{-07}$:
\idxfun{lm}{stats}\idxfun{coef}{stats}\idxfun{mean}{base}\idxfun{paste}{base}\idxfun{cat}{base}\idxfun{solve}{base}
...
The first part of the book was devoted to the OLS estimator, suitable in situations where $\mbox{E}(y_n \mid x_n) = \alpha + \beta^\top x_n$, with $\mbox{V}(\epsilon_n \mid x_n) = \sigma_\epsilon ^ 2$. This model can also be estimated by maximum likelihood by specifying the conditional density of $y$ and not only its conditional expectation. Assuming that $y$ follows a normal distribution, we have:
...
Note that it is block diagonal, as the cross-derivative is $- 2 / \sigma ^ 3 Z ^ \top \epsilon$, which is 0 for the ML estimator.
Solving the last equation in @eq-grad_ml_normal for $\sigma$, we get:

$$
\hat{\sigma} ^ 2 = \frac{\epsilon ^ \top \epsilon}{N}
$$ {#eq-hats2_ml}
...
\idxfun{filter}{dplyr}\idxfun{transmute}{dplyr}\idxfun{mean}{base}
```{r }
aps <- apples %>%
...
\idxfun{as.numeric}{base}\idxfun{logLik}{stats}\idxfun{sum}{base}
...
$$
where $\mu_n$ can equivalently defined as $\mu_n = \alpha +
\sum_{j=1}^J \beta_j \ln q_{nj}^* + \beta_J^* \ln q_{nJ} + \ln q_J$.
...
Denoting $\gamma^ \top = (\alpha, \beta_1, ...\beta_J^*)$ and $z_n = (1, q_{n1} ^ *, q_{n2} ^ *, \ldots q_{nJ-1}^*, q_{nJ})$ and $\epsilon_n = \ln y_n + \lambda y_n - \mu_n$, the derivatives with the unknown parameters $(\gamma, \lambda, \sigma)$ are:
...
To estimate the generalized production function, we use the **maxLik** package [see @HENN:TOOM:11]\index[author]{Henningsen}\index[author]{Hamann}, which is dedicated to the ML
...
\idxfun{maxLik}{maxLik}\idxfun{summary}{base}\idxfun{coef}{stats}
...
The "raw" version of the model can also be estimated by setting the `repar` argument to `FALSE`. In this case, the starting values are the coefficients of `cd_loglm` for all the coefficients but $\lambda$ and the starting value for this latter coefficient is set to 0.
...
that reduces for a "small" model when the hypothesizes are
...
\index[general]{test!three tests principle!maximum likelihood estimator|(}
...
\index[general]{test!three tests principle!linear gaussian model|(}
...
$$
where $\dot{\sigma}_{nc} ^ 2 = \hat{\epsilon}_{nc} ^ \top \hat{\epsilon}_{nc} / (N - K -1)$ is the unbiased estimator of $\sigma ^ 2$. With the hypothesis of iid normal errors, this statistic follows a Fisher-Snedecor distribution with degrees of freedom $J$ and $N-K-1$ and the asymptotic distribution of $J \times F$ is a $\chi ^2$ with $J$ degrees of freedom.
...
These pseudo-R^2^ can be used for any model computed by maximum likelihood and we'll denote them respectively $R^2_{W}$, $R^2_{LR}$ and $R^2_{LM}$. They have been proposed by @MAGE:90\index[author]{Magee}. $R^2_{LR}$ is known as the @COX:SNEL:89's\index[author]{Cox}\index[author]{Snell} R^2^, but has been
...
$R^*$ is then a matrix which select a subset of coefficients, that we'll call $\theta_2$.
...
$$
With the reparametrized model, denoting $\hat{V}_{\hat{\theta}_2}$ the subset of $\hat{V}_{\hat{\theta}}$ that concern $\theta_2$, the Wald statistic simplifies to $W = \hat{\theta}_2 ^ \top\hat{V}_{\hat{\theta}_2}^{-1}\hat{\theta}_2$. The code bellow demonstrates the equivalence of the two approaches:
...
\idxfun{coef}{stats}\idxfun{summary}{base}
...
For the reparametrized model, denoting $g_2$ and $H_2$ the parts of the gradient and of the hessian that concerns $\theta_2$, we get $g_2(-H_2) ^ {-1}g_2$.
...
Applying @eq-lm_formula, or using only the part of the gradient and of the inverse of the hessian that concerns $\theta_2$, we get:
\idxfun{drop}{base}\idxfun{solve}{base}

```{r}
#| collapse: true
lm_test_H <- drop(t(g) %*% solve(- H, g))
g2 <- g[c("log(m)", "lambda")]
H2m1 <- solve(H)[c("log(m)", "lambda"), c("log(m)", "lambda")]
lm_test_H2 <- drop(- t(g2) %*% H2m1 %*% g2)
...
\idxfun{crossprod}{base}\idxfun{solve}{base}\idxfun{drop}{base}

```{r }
#| collapse: true
Ig <- crossprod(G)
lm_test_G <- drop(crossprod(solve(Ig, g), g))
lm_test_G
```
...
\idxfun{diag}{base}\idxfun{coef}{stats}\idxfun{t}{base}\idxfun{vcov}{stats}\idxfun{drop}{base}
...
\idxfun{mean}{base}\idxfun{sqrt}{base}\idxfun{zellner\_revankar}{micsr}\idxfun{sum}{base}\idxfun{apply}{base}\idxfun{attr}{base}
...
\idxfun{exp}{base}\idxfun{print}{base}
...
\idxfun{matrix}{base}\idxfun{apply}{base}\idxfun{sum}{base}
...
\idxfun{crossprod}{base}\idxfun{solve}{base}\idxfun{drop}{base}\idxfun{rep}{base}\idxfun{summary}{base}
...
= 15$ and $K_G = 1$.
\idxfun{vuong\_sim}{micsr}\idxfun{head}{base}\idxfun{mean}{base}\idxfun{abs}{base}
...
@SHI:15\index[author]{Shi} proposed a non-degenerate Vuong test which correct the small sample bias of the numerator of the Vuong statistic and inflates the denominator by adding a constant. 
\index[general]{simulations!Vuong test|)}
...
contributions to the log-likelihood for both models:^[The object returned by `loglm` contains an element called `logLik` that is a vector of individual contributions to the log-likelihood.]
...
\idxfun{length}{base}\idxfun{mean}{base}\idxfun{sqrt}{base}
