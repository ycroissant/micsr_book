...
where $w\overset{a}{=}z$ means that $w$ is asymptotically equal to $z$, ie that they tend to the same limit in probability [see @DAVI:MACK:04, 205].
...
To estimate the generalized production function, we use the **maxLik** package [see @HENN:TOOM:11], which is dedicated to the ML
...
LM / N = \frac{W / N}{1 + W / N}
$$
Denoting $f(x) = \ln (1+x) - x / (1 + x)$, for $x \geq 0$, $f'(x) = x / (1 + x) ^ 2$. Therefore, $f(x)$ is strictly increasing for $x >0$ and, as $f(0) = 0$, $f(x) > 0 \; \forall x >0$. With $x = W / N$, $\ln (1 + W/N) - W/N / (1 + W/N) = LR/N - LM /N > 0$ and therefore $LR > LM$. Therefore, we have proved that:^[see @VAND:81.]
...
These pseudo-R^2^ can be used for any model computed by maximum likelihood and we'll denote them respectively $R^2_{W}$, $R^2_{LR}$ and $R^2_{LM}$. They have been proposed by @MAGE:90. $R^2_{LR}$ is known as the @COX:SNEL:89's R^2^, but has been
previously proposed by @MADD:83. A variant, proposed by @ALDR:NELS:84,  is obtained using the formula of $R^2_{W}$, but using the LR statistic: $LR / (N + LR)$. A problem with the pseudo-R^2^ computed using the LR statistic is that, for a perfect or **saturated** model, it can be lower than 1. Denoting $\ln L_0$ and $\ln L_*$ the values of the log-likelihood for the null (intercept only) and the saturated model, the LR statistic for the saturated model is $LR_* = 2(\ln L_* - \ln L_0)$. In case of discrete response (as for the Poisson model developed in the beginning of this chapter), the individual contributions to the log-likelihood are logs of probabilities (all equal to 1 for a saturated model), so that $\ln L_* = 0$ and $LR_* = -2\ln L_0$. Therefore, $R^2_{LR}$ equals $1 - e^{LR_*/N} \neq 1$. Scaled versions of $R^2_{LR}$ and @ALDR:NELS:84's $R^2$ have been proposed respectively by @NAGE:91:^[And previously used by @CRAG:UHLE:70.]
