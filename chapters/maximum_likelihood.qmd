...
The **maximum likelihood** estimator (**ML** in short) is suitable in situations where the GDP
...
### Computing the ML estimator for the Poisson distribution {#sec-poisson_ml}
\index[general]{Poisson distribution|(}

As an example of count data, we consider the `cartel` data set of @MILL:09\index[author]{Miller} who
...
represents the empirical distribution of the response and add several
...
**likelihood function**. To construct it, it is best to consider the sample as a
...
1 / \theta$ and $\mbox{V}(y) = 1 / \theta ^ 2$ so that,
...
@FAVE:PESA:SHAR:94\index[author]{Favero}\index[author]{Pesaran}\index[author]{Sharma} for which the response `dur` is the time span
between the discovery of an oil field and the date of the British
government approval to exploit this field.
\idxfun{summarise}{dplyr}\idxfun{sd}{stats}
...
estimator is consistent, ie that $\mbox{plim} \,\hat{\theta} = \theta_0$.\index[general]{consistency!maximum likelihood estimator|)}
...
As $\phi = e ^ \lambda$, taking the derivative with 
$\theta$, we get:
...
Taking now the second derivative with $\theta$, we get:
...
We denote $v_\gamma(\theta)$ the first term: evaluated for the true value of the parameter, this is the variance of $\gamma(y; \theta)$:, $v_\gamma(\theta_0) = \sigma_\gamma^2$. We denote the second term $m_\psi(\theta)$, which is, evaluated for the true value of the parameter, the expectation of $\psi(y; \theta)$: $m_\psi(\theta_0) = \mu_\psi$. Therefore, @eq-variance_hessian indicates that $\sigma_\gamma ^ 2 = - \mu_\psi$.
...
This **information based estimator** of the the variance is obtained by
...
\left(\sum_{n=1} ^ N \psi(y_n; \hat{\theta}) \right) ^ 2
= \frac{N\hat{v}_\gamma (\hat{\theta})}{N^2 \hat{m}_\psi(\hat{\theta}) ^ 2} =\frac{\hat{v}_\gamma (\hat{\theta})}{N \hat{m}_\psi(\hat{\theta})} 
$$ {\#eq-sandwich-est}
...
$$
\iota_g(\hat{\theta}) = 
N + \frac{\sum_n y_n ^ 2}{\bar{y} ^ 2} - 2\frac{\sum_n y_n}{\bar{y}}
...
$$
\hat{\sigma}_{\hat{\theta}s} = \frac{N\hat{\sigma}_y ^ 2 / \bar{y} ^ 2}{(N /\bar{y}) ^ 2} = \frac{\hat{\sigma} ^ 2_y}{N}
$$
...
Computing these 4 estimations of the variance of $\hat{\theta}$ for the `ncaught` variable, we get:
\idxfun{nrow}{base}
...
\theta ^ 2 + \sum_{n=1} ^ N y_n ^ 2 - 2 / \theta \sum_{n=1} ^ N y_n 
$$
...
$$
\hat{\sigma}_{\hat{\theta}s}^2 =  \frac{N\hat{\sigma}_y ^ 2}{(N \bar{y} ^ 2) ^ 2}=\frac{\hat{\sigma}_y ^ 2}{N \bar{y} ^ 4}
$$
...
matrix of second derivatives, denoted $\Psi$, which replaces the scalar
...
where $w\overset{a}{=}z$ means that $w$ is asymptotically equal to $z$, ie that they tend to the same limit in probability [see @DAVI:MACK:04, 205]\index[author]{Davidson}\index[author]{McKinnon}.
...
based estimator, also called the **outer product of the gradient** or the **BHHH**^[The initials of the authors of @BERN:HALL:HALL:HAUS:74\index[author]{Berndt}\index[author]{Hall, Browny}\index[author]{Hall, Robert}\index[author]{Hausman} who first proposed this estimator.]
estimator is:
...
Starting from an initial vector of parameters $\gamma_i$, we use a first
order Taylor expansion of the gradient for $\gamma_{i+1}$ "close to"
$\gamma_i$:
...
successive iterations enables to reach a value of $\gamma$ as closed as
...
We then compare the resulting estimated standard errors of the estimator. To obtain a compact output, we use the `sapply` function, which is a specialized version of `lapply`. `lapply` takes as argument a list and a function and the result is the list containing the result of applying the function to each element of the list. `sapply`, when possible, returns a matrix:
...
-\frac{N}{2\sigma} + \frac{1}{\sigma ^ 3} \sum_n (y_n - \gamma ^ \top z_n) ^ 2
\end{array}
\right)
=
\left(
\begin{array}{c}
 ...
 -\frac{N}{2\sigma} + \frac{1}{\sigma ^ 3} \epsilon ^ \top \epsilon
 \end{array}
 \right)
$$ {#eq-grad_ml_normal}

and the ML estimator is obtained by computing the vector of parameters $(\gamma ^ \top, \sigma)$ that set the system of $K+2$ equations to 0. The hessian is:
...
Solving the last equation in @eq-grad_ml_normal for $\sigma$, we get:
...
which makes clear that maximizing the log-likelihood is equivalent to minimizing the sum of square residuals and, therefore, that the ML and the OLS estimators of $\gamma$ are the same. Once $\hat{\gamma}$ has been computed, $\hat{\sigma} ^ 2$ can be estimated using @eq-hats2_ml. Note that the sum of square residuals is divided by the number of observations and not by the number of degrees of freedom, contrary to the OLS estimator. 
\index[general]{normal distribution|)}
\index[general]{linear gaussian model|)}
...
proportional increase of all the inputs is $\sum_{j=1} ^ J \beta_j$ and therefore constant returns to scales imply that $\sum_{j=1} ^ J \beta_j = 1$, or $\beta_J = 1 - \sum_{j=1} ^ {J-1} \beta_j$. Replacing in @eq-cobb_douglas_production, we get:\index[general]{reparametrization!Cobb-Douglass}
...
Denoting $\gamma^ \top = (\alpha, \beta_1, ...\beta_J^*)$, $z_n = (1, q_{n1} ^ *, q_{n2} ^ *, \ldots q_{nJ-1}^*, q_{nJ})$ and $\epsilon_n = \ln y_n + \lambda y_n - \mu_n$, the derivatives with the unknown parameters $(\gamma, \lambda, \sigma)$ are:
...
that reduces to a "small" model when the hypothesizes are
...
log-likelihood under the constrains corresponding to the set of
hypothesis. This leads to three test principles: the likelihood ratio test (based on the comparison of
both models), the Wald test (based only the unconstrained model) and the Lagrange multiplier test (based on the
constrained model). Remind that, applying these tests principles on a model fitted by OLS, the three statistics are exactly
...
We'll consider in this section a set of $J$ linear hypothesis, such that $R\theta - q = 0$ and we'll denote $\hat{\theta}_{nc}$ and $\hat{\theta}_{c}$ the unconstrained and constrained maximum likelihood estimators.

#### The three tests for the linear gaussian model
...
Remind from @eq-const_lm that $\hat{\gamma}_c=\hat{\gamma}_{nc} - (Z^\top Z)^{-1}R^\top A^{-1}(R\hat{\gamma}_{nc}-q)$, then:

$$
\hat{\epsilon}_c = \hat{\epsilon}_{nc}+ Z(Z^\top Z)^{-1}R^\top A^{-1}(R\hat{\gamma}_{nc}-q)
$$ {#eq-resid_c_nc}

and, because $Z ^ \top \hat{\epsilon}_{nc} = 0$, the relation between the two sum of square residuals is:
...
The $R^2$ being equal to $1 - RSS / TSS$, it can be easily expressed, for the linear gaussian model, as a function of the 3 statistics:

$$
R ^ 2 = \frac{W}{N + W} = 1 - e^{LR / N} = LM / N
$$ {#eq-pseudo_r2}

These pseudo-R^2^ can be used for any model computed by maximum likelihood and will be denoted respectively by $R^2_{W}$, $R^2_{LR}$ and $R^2_{LM}$. They have been proposed by @MAGE:90\index[author]{Magee}. $R^2_{LR}$ is known as the @COX:SNEL:89's\index[author]{Cox}\index[author]{Snell} R^2^, but has been
...
and the constant returns to scales hypothesis is 
...
set of parameters is $\theta^ {*\top} = (\alpha, \beta_k, \beta_l, \beta_s^*, \lambda, \sigma)$ and the constant returns to scales hypothesis is $\lambda = \beta_s^*=0$ :
\index[general]{reparametrization!generalized production function}
...
\idxdata[(]{apples}{micsr.data}\idxfun{quad\_form}{micsr}
...
which illustrates the equivalence of the two formulas for the Wald test using either the "raw" or the repametrized model.

We can also use the approximate formula that indicates that the Wald statistic is close to the sum of the squares of the corresponding t statistics if the correlation between the two parameters is not two high:
\idxfun{coef}{stats}
...
For the reparametrized model, denoting $g_2$ and $H_2$ the parts of the gradient and of the hessian that concerns $\theta_2$, we get $g_2^\top (-H_2) ^ {-1}g_2$.
...
derivatives of the log-likelihood with $\theta_1$ should be 0. The other
elements of the gradient are not 0, but should be close to 0 if the
hypothesis are true.  Using the `zellner_revankar` function, we compute all the necessary information (the log-likelihood, the gradient and the hessian) for the constrained model, which corresponds to the coefficients of the `crs` object fitted by least squares using `loglm`:
...
But in this case, the statistic can more simply computed using the
...
The gradient $g$ being the column-wise sum of $G$, it can be written
as $g=G^\top j$, where $j$ is a vector of ones of length $N$.
...
Therefore, regressing $j$ on $G$, the fitted values  are $P_G j$ and the sum of the squares of the fitted
...
\index[general]{coefficient of determination!pseudo-three tests principles|(}
To compute the pseudo R^2^, we should define the "null" or "intercept-only" model. In the generalized production function, it is a model with only two parameters, $\alpha$ and $\sigma$. Therefore, the $R$ matrix select all the coefficients except these two and, using this matrix, we can compute the Wald statistic:
...
### The conditional moment test {#sec-ml_cond_moment_test}
...
test, first presented by @TAUC:85\index[author]{Tauchen} and @NEWE:85\index[author]{Newey} are based on moment conditions that should be 0 under H~0~. They
...
random.^[@CAME:TRIV:05, page 260.\index[author]{Cameron}\index[author]{Trivedi}] Using a first order Taylor
...
Define $V$ as the variance of the vector in parenthesis in @eq-tau_cmtest, obtained by concatenating $g(\theta_0, Z)$ (the gradient for
...
- the first one uses the expected value of the estimators of
  ...
  \theta} / N$,
- the second one uses the same expressions without the expectation:
  $\frac{\partial \ln L}{\partial \theta \partial \theta ^
  \top}(\hat{\theta},Z)/N$ and
  $\frac{\partial m(\hat{\theta}, Z)}{\partial
  \theta} / N$,
- the third one uses the information equality to estimate $\mathcal{H}$
...
But all the columns of $\hat{G}$ sums to 0 ($\hat{G}^\top j=0$),
...
for example @GREE:03\index[author]{Greene}, equation A-74, page 824), the matrix in bracket
in @eq-opg_cmtest.\index[general]{inverse of a partitioned matrix}
...
statistic can be computed using matrix algebra or by taking the explained
...
@VUON:89\index[author]{Vuong} proposed a test for non-nested model. He considers two
competing models characterized by densities $f(y|z; \beta)$ and $g(y|z; \gamma)$. Denoting $h(y | z)$ the true conditional density, the distance of the first model to the true model is measured by the minimum Kullback-Leibler information criterion (**KLIK**):
