...
\idxfun{summary}{base}\idxfun{coef}{stats}
...
\idxfun{summary}{base}\idxfun{coef}{stats}\idxfun{lm}{stats}
...
\idxfun{lm}{stats}\idxfun{summary}{base}\idxfun{coef}{stats}
...
\idxfun{lm}{stats}\idxfun{coef}{stats}\idxfun{summary}{base}
...
\idxfun{coef}{stats}\idxfun{solve}{base}\idxfun{vcov}{stats}\idxfun{t}{base}
...
\idxfun{crossprod}{base}\idxfun{t}{base}\idxfun{sigma}{stats}\idxfun{solve}{base}
...
\idxfun{summary}{base}\idxfun{names}{base}
...
purpose the **Formula** package [@ZEIL:CROI:10]\index[author]{Zeileis}\index[author]{Croissant}. This package provided extended usual formulas in two directions: first, several covariates can be indicated in the left hand side of the formula (using the `+` operator) and several parts can be defined on both sides of the formula, using the `|` sign. For example:
...
\idxfun{model.part}{Formula}\idxfun{pivot\_longer}{dplyr}\idxfun{print}{base}
...
interface. 
\idxfun{bind\_cols}{dplyr}\idxfun{print}{base}
...
estimation and provide a `systemfit`\idxfun{systemfit}{systemfit} function. Its main arguments are a list of equations and a data
