...
Finally, let's denote $\ln A(0) = a + \epsilon$. $\epsilon$ is an error term which represent the initial dispersion between
...
$\ln i_n$ (the saving rate) and $\ln v_n$ (the demographic rate). Moreover, the structural model imposes some
...
@MANK:ROME:WEIL:92 proposed a generalization of the Solow-Swan model
...
`growth` contains the data used by @MANK:ROME:WEIL:92. It
...
Dividing the the first line of @eq-multiple_ols_foc by the sample size, we get:
...
@eq-multiple_ols_foc_covariates can then be written in matrix form:
...
Equating @eq-ellipse to this critical, we get the equation of an ellipse, with two particular nested cases:
...
The critical values being much larger that the computed statistics, the
...
a **constrained model**, which imposes the hypotheses and an **unconstrained model**, which doesn't impose the hypotheses.
...
Moreover, the cost shares sum to 1 whatever the value of the price, so that
...
purpose the **Formula** package [@ZEIL:CROI:10]. This package provided extended usual formulas in two directions: first, several covariates can be indicated in the left hand side of the formula (using the `+` operator) and several parts can be defined on both sides of the formula, using the `|` sign. For example:
...
beta_c
```
More simply, `micsr::clm` can be used, which computes the constrained
