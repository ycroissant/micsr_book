...
to the previous two chapters, we'll insist on two features of the
multiple regression model:
...
We'll roughly follow the same plan as for the simple linear model: we'll
...
estimator (from a theoretical point of view and with **R**), the
geometry of the multiple linear model, its statistical properties and
the inference methods (confidence interval and tests).
...
On of the most popular growth model in the economic litterature is the Solow-Swan model.
...
Replacing $k(t)$ by $k^*$ and taking logs, we get:
...
multiplicative error term which represent the initial dispersion between
...
$\ln i_n$ (the saving rate) $\ln v_n$ (the demographic rate). Moreover, the structural model imposes some
...
$\beta_v = -\frac{\kappa}{1 - \kappa}$ and
$\beta_i = \frac{\kappa}{1-\kappa}$. Moreover, $\kappa$ is the
...
similar to the one we previously developped. We first compute the growth rate of physical
...
The data set used is `micsr.data::growth` and is the one used by @MANK:ROME:WEIL:92. It
...
of the points proportional to GDP per capita.
...
For one observation, denoting $x_{n1}$ and $x_{n2}$ the two covariates, the model is:
...
Note that $\tilde{I}$ is **idempotent**, which means that $\tilde{I} \times \tilde{I}$. It can be checked that using direct multiplication, but also by reminding that premultiplyong a vector by $\tilde{I}$ removes the sample mean of the values of the vector. The transformed vector then have a zero mean, so that applying the same premultitplication one more time will leave it unchanged. Therefore, $\tilde{I}(\tilde{I} z) = \tilde{I}z$. Denoting $\tilde{z} = \tilde{I} z$, we get:

$$
\tilde{X}= \tilde{I} X =
\left(\begin{array}{cc}
x_{11} - \bar{x}_1 & x_{21} - \bar{x}_2\\
x_{12} - \bar{x}_1 & x_{22} - \bar{x}_2\\
\vdots & \vdots \\
...
The first order conditions can then be written in matrix form:
...
To compute the estimator, we need to inverse to compute the inverse of $\tilde{X} ^ \top \tilde{X}$, which is:
...
- the two covariates are positively correlated (males are more educated than females in average). In this case $\hat{\rho}_{2y}/(\hat{\rho}_{12}\hat{\rho}_{1y})>1$ and the numerator of @eq-mult_simple is lower than $1 - \hat{\rho}_{12} ^ 2$, so that $\hat{\beta}_1<\hat{\beta}_1^S$. $\hat{\beta}_1 ^ S$ is upward biased because it estimates the sum of the positive direct effect of education on wage and a positive indirect effect (more education leads to a subpopulation with a higher share of males and therefore higher wages),
...
-   $\gamma^\top = (\alpha, \beta ^ \top)$ the vector of parameters
    obtained by adding the intercept $\alpha$ to the vector of slopes:
...
matrix transforms the vector of response as a vector of prediction. As
...
that are square and symetric of dimension $N \times N$, which means
that they are in practice very large matrices, and are therefore never
...
$P_X y = \hat{y}$, the $y$ vector is projected in the subspace spanned by
...
operator. Actually, formulas have a much richer syntax that includes other operators, like for example `*` and `:`. This will be discussed in [Chapter -@sec-interpretation_chapter].
...
The estimator can also be computed "by hand", using matrix algebra. To start, we use the `model.frame` function which, as `lm` `formula` and  `data` arguments. For pedagogical purpose, we add the `group` variable in the formula.
...
`model.frame`. The $Z$ matrix is obtained using the `model.matrix` function, which also use a 
...
The efficient method used by `lm` to compute the OLS estimate is the QR decomposition:^[see @DAVI:MACK:93, section 1.5, pp. 25-31] the matrix of covariate $Z$ can be written as the product of an orthonormal matrix $Q$ of dimension $N \times (K + 1)$ (therefore $Q^\top Q = I$) and an upper triangular matrix $R$ of dimension $(K+1) \times (K+1)$. Then, the linear model can be written as:
...
and $\hat{\gamma}$ is obtained by solving recursivelly $\hat{\delta} = R \hat{\gamma}$:
...
In this section, we'll briefly analyze the statistical properties of the OLS estimator with more than one covariate. Most of these properties are similar to the one we have described on the previous chapter. They'll be presented in this section using matrix algebra.

### The unbiasness of the OLS estimator

The vector of slopes can be written as a linear combination of the response vector
...
- homoscedasticity: $\mbox{E}(\epsilon_n ^ 2 \mid x) = \sigma_\epsilon ^ 2$ and
...
We now go back to the estimation of the growth model. As @MANK:ROME:WEIL:92, we use a restricted sample by excluding countries for which most part of the GDP is linked to oil extraction and those with low quality data. 
...
The `summary` method computes detailed results of the regression, in particular the table of coefficients, which can be extracted using the `coef` method:
...
- the second one correspons to a reasonable value of the share of profits, which is approximatively one third; therefore we'll test the hypothesis that $\kappa = 1/3$, which implies that $\beta_i = 0.5$.
...
Confidence interval and tests for a coefficient are therefore computed exactly as for the case of the simple linear regression model. In particular, using @eq-var_covar_slopes, we get:

$$
t_k = \frac{\hat{\beta}_k - \beta_k}{\hat{\sigma}_{\hat{\beta}_k}} = 
\frac{\hat{\beta}_k - \beta_k}{\frac{\hat{\sigma}_\epsilon}{\sqrt{N}\hat{\sigma}_1\sqrt{1 -\hat{\rho}_{12}^2}}}
$$ {#eq-student_multiple}

follows exactly a Student distribution with $N-K-1$ degrees of freedom if the errors are normal and asymptotically a normal distribution whatever the distribution of errors. Therefore, $1 - \alpha$ confidence intervals are: $\hat{\beta}_k \pm \mbox{cv}_{1-\alpha / 2} t_k$ where $\mbox{cv}_{1-\alpha / 2}$ is the critical value of either a Student or a
normal distribution. For a given hypothesis: $H_0:\beta_k = \beta_{k0}$,
$t_{k0}=\frac{\hat{\beta}_k - \beta_{k0}}{\hat{\sigma}_{\hat{\beta}_k}}$
...
The hypothesis is therefore not rejected, even at the 10% level. Once again, such a linear hypothesis can be more easily tested using a different parametrization. Introducing in the model the term: $(\beta_i + \beta_v) \ln i$, substracting $\beta_v \ln_i$ and re-arranging terms:
...
\end{array}
$$
We then have a model for which the two covariates are now $\ln i$ and $\ln v / i$, the hypothesis being that the coefficient associated of $\ln i$ is equal to 0.
...
In the simple linear case, subtracting the expected value and dividing by the standard deviation, we get a standard normal deviate. Taking the square, we get a $\chi^2$ with one degree of freedom. If the $K$ slopes are uncorrelated: $\sum_{k=1} ^ K\frac{(\hat{\beta}_k - \beta_{k0}) ^ 2}{\hat{\sigma_{\hat{\beta}_k}} ^ 2}$
is a $\chi^2$ with $K$ degrees of freedom. 
...
red the point that corresponds to the hypothesis $\kappa = 1/3$, which
...
The confidence ellipse is a "fat sausage"^[@STOC:WATS:15, p. 235.] with the long part of the
...
-   $(\beta_i = 1.25, \beta_v = -3.1)$ represented by a triangle point:
    ...
    hypothesis $\beta_b = -3.1$ is, but the joint hypothesis
...
full data set because of missing data and because the estimation is performed exclude some groups of countries:
...
contains parenthesis, therefore they should be surrounded by the ` ` ` sign. The
...
Consider the constrained model, which impose $\beta_2 = 0$. It is therefore obtained by regressing $y$ on $X_1$ only and the vector of residuals is $\hat{\epsilon}_1 = M_1 y$. The idea of this test is that, if $H_0$ is true, $X_2 ^ \top \hat{\epsilon}_1$
...
The second hypothesis implies that $\beta_i = \frac{1/3}{2/3 - \lambda}$ and therefore that $\beta_e = 2 \beta_i - 1$. The model can be re-parametrized in such way that two slopes are 0 if the two hypothesis are satisfied:

$$
y + e - v = \kappa + \beta_i (i + 2e - 3v) + (\beta_e-2\beta_i+1)(e-v)+(\beta_i + \beta_e+\beta_v)v
$$

Therefore, the unconstrained model can be written as a model with $y + e - v$ as the response and $i + 2 e - 3 v$, $e-v$ and $v$ as the three covariates and the constrained model as a model with the same response but with $i + 2 e - 3$ as unique covariate.
...
Both hypothesis are not rejected, even at the 10% level. To test the join hypothesis, we can first use the approximative formula which is the sum or the mean of the squares of the t statistics (respectively a $\chi^2$ with 2 degrees of freedom and a F with 2 and 71 degrees of freedom):
...
`car::linearHypothesis` performs Wald tests with a nice syntax: the first argument is a fitted model and the second one is a vector of character which enables simply to write the hypothesis:

```{r}
#| label: linear_hypothesis
car::linearHypothesis(mrw2, c("i + v + e = 0", "e = 2 * i - 1"))
```

#### Likelihood ratio test

The computation of the likelihood ratio statistic is very simple once the two models have been estimated. The sum of squares of the two models are extracted using the `deviance` method and we divide the difference of the two sums of squares by $\dot{\sigma}_\epsilon^2$ (the `sigma` method is used to extract the standard deviation of the residuals) and by 2 to get a F statistic.
...
For the score test, we consider the re-parametrized model and we define $1_1$ as a vector of one and $i + 2  e - 3  v$ and $X_2$ as $e-v$ and $v$.

```{r}
#| label: score_test_model_matrix
X2 <- model.matrix(~ e + v - 1, model.frame(mrw2))
Z1 <- model.matrix(mrw_c)
```
...
The interest of this testing strategy is not limited to the test that all the slopes of a real model is 0. It can also be used to test any set of hypothesis, using re-parametrization and the Frisch-Waugh theorem. Consider for example the hypothesis that $\beta_i = 0.5$ and $\beta_i + \beta_e + \beta_v=0$. We have seen previously that, after re-parametrization, this corresponds to the model:
...
with, if the two hypothesis are true, the two slopes associated with `e2` and `v` equal to 0. Now, using the Frisch-Waugh theorem, and denoting $X_1 = (j, i_2)$ and $X_2 = (e_2, v)$:
...
indicating that the demands for input are homogeneous of degree 0 in
...
We estimate the translog cost function, with the `apples` data set of
...
`Formula::model.part` enables to retrieve any part of the model. Here, we want to extract the three responses which are the only left side of the formula, so we set `lrs` and `rhs` respectively to 1 and 0. The result is a data frame with three variables `ct`, `sl` and `sm`. We then use `dplyr::pivot_longer`, to stack the three responses in one column. Note the use of the optional argument `cols_vary` that is set to `"slowest"`, so that the elements are stacked columnwise:
...
with $\epsilon=y - Z\gamma$ and $\lambda$ the vector of Lagrange
...
Finally, the **systemfit** package [@HENN:HAMA:07] is devoted to system
estimation and provide a `systemfit` function. Its main arguments are a list of equations and a data
...
systemfit::systemfit(list(cost = eq_ct, labor = eq_sl, materials = eq_sm),
