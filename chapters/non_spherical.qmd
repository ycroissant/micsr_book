...
In the first part of the book, we considered a model of the form: $y_n = \alpha + \beta x_n + \epsilon_n$
with two fundamental hypothesis:
...
The first hypothesis, that implies that the error are uncorrelated with the covariates will be maintained in this chapter and ensures that the OLS estimator is still unbiased and consistent. The second hypothesis is the hypothesis of constant conditional variance of the response (or constant variance of the errors). Along with the hypothesis are uncorrelated ($\mbox{vcov}(\epsilon_n, \epsilon_m) = 0 \; \forall n\neq m$). These hypothesis of **homoskedastic** and **uncorrelated** errors implies that the covariance matrix of the errors $\Omega$ is, up to a multiplicative constant $\sigma_\epsilon^2$, the identity matrix:

$$
\Omega = \mbox{V}(\epsilon) = \mbox{E}(\epsilon\epsilon^\top) = \sigma_\epsilon ^ 2 I
$$

In this chapter, we analyze cases where this hypothesis is violated. This has the following consequences concerning the results establieshed in the first part of the book:

-   the OLS estimator is still consistent: this means that
    $\hat{\beta}$ estimates consistently $\beta$ and $\hat{\epsilon}$
    estimates also consistently $\epsilon$; this is an important result as the
    residuals of the OLS estimator can therefore be used to test whether the errors are
    spherical or not,
-   the OLS estimator is no longer BLUE, ie it is no longer the best linear
    unbiased estimator,
-   there is another linear unbiased estimator (the **generalized least squares**, abbreviated as **GLS**) which is more efficient (which means that it has a smaller variance) than the OLS estimator and which is the BLUE estimator when the errors are non-spherical.
...
@sec-situations_non_spher reviews some important cases where the errors are
non-spherical. @sec-sandwich presents robust estimators of the variance
of the OLS estimators. Section @sec-gls is devoted to the GLS
estimator. Finally section @sec-test_non_spher presents tests that enables to detect
...
situations in the next three subsections and we'll establish the features of the matrix of covariance of the errors $\Omega$. Moreover, for a reason that will be clear in the subsequent sections, we'll compute the inverse of this matrix.
...
heteroskedasticity often occurs when the response $y$ is scaled. Consider for example the case where the response is the average consumption of a good in a different geographical entities (cities for example). Then, denote $c_i$ the individual consumption, then the response is $y_n = \frac{1}{I_n} \sum_i c_{ni}$, where $n$ is a city and $I_n$ the number of consumer units in this city. Then, if the standard deviation of the individual consumption is $\sigma_c$ (the same in every city), the variance of $y_n$ is $\sigma_c ^ 2 / I_n$ and therefore depends on the number of consumer unit of the city, even if the variance of the individual consumption is the same in every city. Even if covariates are taken into account, it is doubtful that the conditional variance of $y$ will be the same for every city and a more reasonable hypothesis is that, as its unconditional variance, it is inversely proportional to the number of consumer units. The matrix of covariance of the disturbances is therefore a diagonal matrix with
...
@BONJ:CHERK:KASK:03 studied the return to education with a sample of twins.
Therefore, the sample contains 214 observations (107 couples of twins).
...
The error of the model can be written as the sum of two components:
...
-   the two components are homoskedastic and uncorrelated:
    $\mbox{V}(\eta_n) = \sigma_{\eta} ^ 2$,
    $\mbox{V}(\nu_{nt}) = \sigma_{\nu} ^ 2$ and $\mbox{cov}(\eta_n, \nu_{nt}) = 0$,
...
-   the two components of the errors are uncorrelated for two
    ...
    $\mbox{cov}(\nu_{nt}, \nu_{ms})=\mbox{cov}(\eta_n, \eta_m)=0\; \forall \; m \neq n$
...
The blocks of this matrix can be writen as, denoting $j$ a vector of ones and $J=jj^\top$ a square matrix of 1:
...
by $B$ transforms it as a vector of individual means. These two symetric matrices
...
-   they sum to the **identity matrix**, $W + B = I$. $Wz + Bz=z$
    because the sum of the deviation from the individual means of a vector and its individual means is the vector itself.
...
-   secondly, economic theory may impose restrictions on different
    ...
    constrained least squares.^[See @sec-system_equation.]
...
The inverse of the covariance matrix of the error is easily obtained as it requires only to compute the inverse of $\Sigma$:
...
If the errors are not spherical, the simple estimator of the
covariance of the OLS estimates is biased. More general estimators
can then be used instead. These estimators use the residual of the OLS estimator which, in the context of this chapter, is a inefficient but consistent estimator. We'll present the robust estimator first in the context of the simple linear model and then for the multiple linear model.
...
The numerator is the sum of the expectations of $N ^ 2$ terms. For $N
= 4$, replacing the errors $\epsilon_n$ by the **OLS** residuals
$\hat{\epsilon}$ and dropping the expectation operator, these 16 terms
...
$\left(\sum_{n = 1} ^ N (x_n - \bar{x})\hat{\epsilon}_{n}\right)^2$, which is
equal to zero as: $\sum_{n = 1} ^ N (x_n - \bar{x})\hat{\epsilon}_n=0$.
...
which is relevant if we maintain the hypothese that the errors are uncorrelated. In
this case, we get the so-called **heterosckedastic-consistent** (or **HC**)
estimator of $\sigma_{\hat{\beta}}$ first proposed by @WHIT:80:
...
common unobserved features of the regions. As an example, in our 4
...
estimator is obtained by summing the following subset of elements of the preceding
matrix:
...
which leads to the clustered estimated variance. More generally, for
$N$ observations belonging to $G$ groups, this estimator is:

$$
\hat{\sigma}_{CL\hat{\beta}} ^2= \frac{1}{S_{xx}^2}\sum_{g = 1} ^ G \left(\sum_{n \in g} (x_n -
...
population, the area and the distance to the central business district for 2315 block
groups for Alabama.
...
district : $y = A e^{\beta x}$ where $y$ is measured in inhabitants
per square kilometers and $x$ is measured in kilometers and $\beta<0$
is called the urban gradient. Taking logs, this leads to a semi-log
...
ug_ols %>% summary %>% coef %>% print(digits = 3)
```

The estimated standard deviation of the slope is `r
round(vcov(ug_ols)[2, 2], 4)` but it may be seriously biased if the
errors are heteroskedastic and/or correlating. We first plot the data
and the regression line on @fig-dataug, the color of the points depending of the metropolitan statistical area (there are 12 of them in the sample).
...
Heteroskedasticity seems to be present in this data set, as the size
of the residuals seems to be an increasing function of the unique
covariate. We first compute the robust standard deviation of the slope
by first computing the mean of the covariate, the sum of squares of
the covariate and finally the standard deviation.
...
`type = "HC0"` is the simplest version of the estimator, the one we
have previously computed "by hand". Other flavors of this
heteroskedasciticy-robust estimator can be obtained by setting the
`type` to `"HC1"`, $\ldots$, `"HC5"` to perform some kind of degrees
of freedom correction. For example, when `type = "HC1"`, the
covariance matrix is multiplied by $N / (N - K)$:^[See @ZEIL:06 for
more details concerning the other values of the `type` argument.]
...
heteroskedastic-robust estimation of the standard deviation of the
slope is small. However, we also have to investigate the potential correlation
between the errors of some observations.

There are 12 msa in Alabama and 22 counties. It is possible that
the errors for the different block groups of the same county or the
same msa are correlated (because of some unobserved common features of
block groups in the same county or msa). In this case the covariance
matrix of the OLS estimates are biased. We first compute the estimation
of the clustered standard deviation of the slope at the msa level using @eq-vcovHC:
...
This time, we get a much higher estimate of the standard deviation
(about three time larger than the one obtained with the simple
formula). The `sandwich::vcovCL` function 
computes the clustered covariance matrix. The clustering variable is
defined using the `cluster` argument that can be set to a one-sided
formula. The `type` argument is similar to the one of `vcovHC` and
there is also a `cadjust` argument which, if `TRUE`, multiplies the
covariance matrix by $G / (G - 1)$, $G$ being the number of clusters.

```{r}
sqrt(vcovCL(ug_ols, type = "HC0", cadjust = FALSE, cluster = ~ msa)[2, 2])
```

The default behaviour of `vcovCL` is to set `cadjust` to `TRUE` and
...
Consider now the multiple regression model. The vector of slopes can be written as a linear combination of the vector of response, and then of the vector of error ($\tilde{X}$ means that each covariate is measured as deviation from the sample mean):
...
$\tilde{X} ^ \top\epsilon$ is a $K$-length vector containing the product of
...
= \sum_{n = 1} ^ N \psi_n
$$
$\psi_n$ is called the vector of score, as it is proportional to the vector of first derivatives of the sum of squares residuals and is therefore equal to 0 when evaluated for $\hat{\beta}$, the OLS estimator. The general form of the covariance of the OLS estimates was given in @eq-general_variance:
...
This is a sandwich formula, the meat being surrounded by two slices of bread which are the inverse of the covariance matrix of the covariates. Remind from @sec-variance_ols that the meat is the variance of the vector of score, and can be written, for $K = 2$, as the expected value of:
...
To get the clustered estimator of the variance, we define for
...
```{r}
#| include: false
growth <- micsr.data::growth
```

## Generalized least square estimator {#sec-gls}

For a linear model $y_n = \alpha + \beta^\top x_n + \epsilon_n = \gamma ^ \top z_n + \epsilon_n$ with non-spherical
disturbances ($\Omega \neq \sigma_\epsilon ^ 2 I$), the OLS estimator is now longer BLUE. A more efficient estimator called the generalized least square can then be used instead.
...
A feasible GLS estimator is obtained by imposing some structure on $\Omega$ so that the number of unknown parameters became much less than $N(N+1)/2$ and by estimating these unknown parameters using residuals of a first step consistent estimation.
Actually, in practice, the GLS estimator is obtained by performing
...
$$
\hat{\gamma} = (Z^\top C^\top CX) ^ {-1} Z^\top C^\top C y = \left(Z^{*\top}Z^*\right)^{-1} Z^{*\top}y^*
$$ {#eq-glstrans}
...
But $C\Omega^{-1}C^\top = C (C^\top C)^{-1} C^\top = C C ^ {-1} C^{\top-1}C^\top=I$[^non_spherical-2],
...
a smaller weight in WLS compared to OLS. The weights
...
formula that can be used to compute any power of $\Omega$ and $C$ is in this context
...
[@BREU:PAGA:79] consider the following heteroskedastic model:
...
$$ 
\frac{\partial \ln ^ 2 L}{\partial \alpha \alpha ^ \top} =
\frac{1}{2}\sum_n\left[ h_n''\left(\frac{e_n ^ 2}{\sigma_n ^ 4}-
\frac{1}{\sigma_n ^ 2} \right)- h_n^{'2} \left(\frac{2 e_n ^
2}{\sigma_n ^ 6}- \frac{1}{\sigma_n ^ 4} \right)\right]z_n z_n'
$$
...
can be considered as a specialized case of @BREU:PAGA:79's test.
...
To get the unconditional density, we have to integrate out this
conditional density with $\eta$. Assuming that $\eta$ is normal with
zero expectation and a variance equal to $\sigma_\eta ^ 2$, we get:
...
estimation of the system of three equations of three equations (one
cost function and two factor shares equations) for the production of
apples started in @sec-system_equation.
...
We then computed a data frame containing the stacked data for
the whole system:

```{r }
stack_data %>% print(n = 2)
```
and we were able to estimate the unconstrained and the
...
```
and to extract matrices of covariates for the three equations (`X_c`, `X_l` and `X_m`).


```{r }
X_c <- model.matrix(eq_ct, ap) ;
X_l <- model.matrix(eq_sl, ap) ;
X_m <- model.matrix(eq_sm, ap)
```

To implement the **SUR** estimator, we first estimate the matrix of
covariance of the errors for the three equations of the system, using
the **OLS** residuals:

```{r }
N <- nobs(ols_const) / 3
EPS <- ols_const %>% resid %>% matrix(ncol = 3)
Sigma <- crossprod(EPS) / N
Sigma
```

We then compute the estimated standard deviation of the errors for the
two equations and we compute the Breusch Pagan test:
...
The hypothesis of diagonality of $\Sigma$ is rejected at the 1% level
(but not at the 0.1% level).

We then compute the Cholesky decomposition matrix of this covariance
matrix and construct the matrix of transformed covariates and
response for the whole system:

```{r }
V <- chol(solve(Sigma))
Xs <- rbind(cbind(V[1, 1] * X_c, V[1, 2] * X_l, V[1, 3] * X_m),
            cbind(V[2, 1] * X_c, V[2, 2] * X_l, V[2, 3] * X_m),
            cbind(V[3, 1] * X_c, V[3, 2] * X_l, V[3, 3] * X_m))
ys <- as.matrix(Y) %*% t(V) %>% as.numeric
```

Then the **SUR** estimator is computed, using `lm` on the transformed
data and the using `¢lm` in order to impose the linear restrictions.

```{r }
sur <- lm(ys ~ Xs - 1) %>% clm(R = R)
```



```{r }
sur <- systemfit::systemfit(list(cost = eq_ct, labor = eq_sl, materials = eq_sm),
...
is the elasticity of the demand of labor with the price of
materials. The price elasticities indicates that the demand for the
three inputs is inelastic, and it is particularly the case for labor
and materials..
