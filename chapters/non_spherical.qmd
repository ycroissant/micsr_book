...
$\mbox{V}(\epsilon) = \sigma_\epsilon ^ 2 I$, which implies that:
...
can then be used instead. They rely on the idea that:
...
We present the robust estimator first in the context of the simple
linear model and then for the multiple linear model.
...
population, the area and the distance to the **CBD** for 2315 block
groups for Alabama.
...
and the regression line on @fig-dataug.
...
slope is small. However, we also have to invistigate the potential
...
matrix of the **OLS** estimates are biased. We compute the estimation
of the clustered standard deviation of the slope at the **msa** level.
...
formula). The `sandwich` package provides a `vcovCL` function for the
computation of clustered covariance matrix. The clustering variable is
