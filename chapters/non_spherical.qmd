...
$y_n = \alpha + \beta x_n + \epsilon_n$ with three fundamental
hypothesis:
:::
...
\left\{
\begin{array}{rcl}
\mbox{E}(y_n \mid x  = x_n) &=& \alpha + \beta x_n  + \mbox{E}(\epsilon_n \mid x = x_n) = \alpha + \beta x_n\\
\mbox{V}(y_n \mid x  = x_n) &=& \mbox{E}\left((y_n - \alpha - \beta x_n) ^ 2\right) = \mbox{E}(\epsilon_n^2) = \mbox{V}(\epsilon_n) = \sigma_\epsilon ^ 2 \\
\mbox{E}(\epsilon_n \epsilon_m) &=& 0 \; \forall n \neq m
\end{array}
\right.
$$

The first hypothesis, that implies that the error are uncorrelated with
the covariates will be maintained in this chapter and ensures that the
OLS estimator is still unbiased and consistent. The second hypothesis is
the hypothesis of constant conditional variance of the response (or
constant variance of the errors) and the third one is the hypothesis of
uncorrelated errors. In this case, the errors (or disturbances) are
...
In this chapter, we analyze cases where this hypothesis is violated.
...
-   there is another linear unbiased estimator (the **generalized least
    ...
    which is the BLUE estimator when the errors are non-spherical,
-   the simple formula for the variance of the OLS estimator,
    ...
    **sandwich** estimators, which are unbiased, can be used.
...
Throughout this chapter, we'll use intensively the following libraries:

- **sandwich** (computation of robust covariance matrices),
- **plm** (panel data),
- **systemfit** (system of equations),
...
homoscedastic and uncorrelated errors. We'll describe in the next three subsections important situations where this hypothesis is violated. In each case, we'll establish the
...
As an example, @HOUT:51 analyzed electric consumption in the United Kingdom and his data set (called `uk_elec`) is a sample of 42 British cities. The response is the per capita consumption in kilowatt hours. Denoting $c_{ni}$ the consumption of an individual $i$ in city $n$, the response is then $y_n =\frac{1}{I_n}\sum_i^{I_n} c_{ni}$ where $I_n$ is the total number of consummers in city $n$. Then, if the standard
...
inversely proportional to the number of consumer units. With heteroscedastic but uncorrelated errors, the matrix of covariance of the errors is a diagonal matrix with non-constant diagonal terms:
...
observation is the individual, but seeblings are observed. In this case, each observation is doubly indexed,
...
is from @BONJ:CHERK:KASK:03 who studied the return to education with a
...
The second one, called `tobinq`, is from @SCHA:90 who tested the relevance of Tobins' Q theory of investment by
...
firm and the stock of capital. The data set is a panel of 188 firms observed for 35 years (from 1951 to 1985).
...
so-called **error-component** model, which can be easily analysed with the following
hypothesis:

-   the two components are homoscedastic and uncorrelated:
...
The blocks of this matrix can be writen as, denoting $j$ a vector of
...
-   they sum to the **identity matrix**, $W + B = I$. $Wz + Bz=z$
    because the sum of the deviation from the individual means of a
...
for example in the fields of the micro-econometrics of
consumption or production, it is more relevant to consider the estimation of system of equations instead of the estimation of a single equation. In matrix form, the model corresponding to the whole system was presented in @eq-system_equation. We've seen in @sec-system_equation than a first advantage of considering the whole system of equations, and not the different equations in isolation was that restrictions on coefficients that concern different equations can be taken into account using the method of constrained least squares.
...
Denoting $\epsilon_l$ the vector of lenght $N$ containing the errors for the $l$^th^ equation and $\Xi = (\epsilon_1, \epsilon_2, \ldots \epsilon_L)$ the $N\times L$ matrix containing errors for the whole system, the covariance matrix of the errors of the system is:
...
$\sigma_{lm}$, is constant. The variance of errors for each equation $l$ is denoted $\sigma_{ll}$ and may be different from one equation to another. Moreover, we assume that errors for different individuals are uncorrelated. With these hypothesis, the covariance matrix is:
...
Denoting $\Sigma$ the $L\times L$ matrix of inter-equations covariances, we have:
...
and $\Omega=\Sigma \otimes I$.

The inverse of the covariance matrix of the error is easily obtained as
it requires only to compute the inverse of $\Sigma$:

$$
\Omega ^ {-1} = \Sigma ^ {-1} \otimes I
$$
...
present a family of tests that are based on OLS residuals. These tests
are relevant because even if the disturbances are non-spherical, OLS is
...
residuals to analyse the unknown features of the errors.

### Testing for heteroskedasticity

[@BREU:PAGA:79] consider the following heteroskedastic model: $y_n = \gamma ^ \top z_n + \epsilon_n$ with $\epsilon_n \sim \mathcal{N}(0, \sigma_n ^ 2)$. Assume that $\sigma_n^2$ is a
function of a set of covariates denoted $w_n$:

$$
\sigma_n ^ 2 = h(\delta ^ \top w_n)
$$ 

The first element of $w$ is 1, so that the homoscedasticity
...
With the homoscedasticity hypothesis, $\sigma_n = \sigma$ and $h'_n = h_n'(\delta_0) = h_0$ and @eq-gen_score_bp_heter simplifies to:
...
of this matrix. If the errors are homoscedastic, the first term disappears and $h'_n = h'_0$ so that:
...
$w_n$. Note also that
$f'f / N = \sum_n \left(\frac{\hat{\epsilon}_n ^ 2}{\hat{\sigma} ^ 2} - 1\right) ^ 2 / N= \sum_n\left(\frac{\hat{\epsilon}_n ^ 4}{\hat{\sigma} ^ 4} + 1 - 2 \frac{\hat{\epsilon}_n ^ 2}{\hat{\sigma} ^ 2}\right) / N$
is the total sum of squares divided by $N$ which converges to
2.[^non_spherical-5] Therefore, the statistic can also be computed as
$N$ times the R^2^ of a regression of the first-step residuals on $w$:

[^non_spherical-5]: The first term is the fourth center moment of a
    normal variable, which is 3.

$$
N R^2 = \frac{f^\top W (W^\top W)^{-1} W f}{f'f / N}
$$

@WHIT:80 proposed a test that is directly linked to its proposition of
...
asymptotically distributed as a $\chi ^ 2$. Therefore, @WHIT:80's test
can be considered as a special case of @BREU:PAGA:79's test.

In his electric consumption regression, @HOUT:51 used as covariates `inc` (average yearly income in pounds), the inverse of `mc6` (the marginal cost of electricity), `gas6` (the marginal price of gas) and `cap` (the average holdings of heavy electric equipment). The OLS estimation is:
...
To use @eq-bp_heteros_ESS, we need to compute from this regression $f_n$ and $W$
...
And we then regress $f$ on $W$ (which is here a constant and the inverse of `cust`) and get half the explained sum of squares:
...
The homoscedasticity hypothesis is therefore highly rejected.
The `lmtest::bptest` computes automatically the Breusch-Pagan test for heteroscedasticity, with two formulas: the first one being the formula of the model and the second one being a one-side formula for the scedasticity equation. An alternative syntax is to provide a `lm` model as the first argument:
...
Note that we set the `studentize` argument to `FALSE`. The default value is `TRUE` and in this case, a modified version of the test due to @KOEN:81 is used.

### Testing for individual effects

@BREU:PAGA:80 extend their Lagrange multiplier test for detecting
...
the estimator for the parameters is the one obtained using OLS and 
$\hat{\sigma}_\nu^2$ is $\hat{\epsilon}^\top\hat{\epsilon} / NT$. The
score and its estimated variance are then, with $\hat{\theta}^\top = (\hat{\sigma}_\nu^ 2, 0)$
...
$$ LM = \left(-\frac{NT}{2\hat{\sigma}_\nu^2}
  \left(1 - \frac{\hat{\epsilon}^\top B\hat{\epsilon}}{N\hat{\sigma}_\nu^2}\right)\right)^2
\times \frac{2\hat{\sigma}_\nu^4}{NT(T-1)} =
\frac{NT}{2(T-1)}\left(1-\frac{\hat{\epsilon}^\top B\hat{\epsilon}}{N\hat{\sigma}_\nu^2}\right)^2
$$ 
...
@BONJ:CHERK:KASK:03 estimated a Mincer equation of twins, the entity index being `family`. The response is the log of wage and the covariates are education (`educ`) and potential experience and its square (approximated by the age `age`):
...
The test can easily be obtained using `plm::plmtest`. We set the `type` argument to `"bp"` to get the statistic of the original Beush Pagan test:

```{r}
#| label: bp_twins_plmtest
library(plm)
plmtest(log(earning) ~ poly(age, 2) + educ, twins, type = "bp")
```

The absence of individual effects is rejected at the 5% level, but not at the 1% level.

The model fitted by @SCHA:90 is a simple linear model, the response being the rate of investment (`ikn`)  and the unique covariate Tobin's Q (`qn`):
...
The first two columns contain the firm and the time index. The Breush-Pagan statistic is:

```{r}
plmtest(ikn ~ qn, tobinq, type = "bp")
```

The statistic is huge, the hypothesis of no individual effects is therefore very strongly rejected, which is a quite customary result for panel data sets, especially when the time dimension is high, which is the case for the `TobinQ` data (35 years).
...
\right)
$$
The coefficients of correlations are then estimated, denoting $\hat{\sigma} ^ 2_l = \epsilon_l^\top \epsilon_l/ N$ the estimated covariance:
...
so that we get a matrix with ones on the diagonal and coefficients of correlations off-diagonal. Then, we extract the off-diagonal elements using `upper.tri`, ^[`upper.tri` returns a vector containing the elements above the diagonal, as the matrix is symetric, `lower.tri` could also have been used.] we sum their squares and multiply by the sample size to get the statistic:
...
c(bp_stat, pval_pb)
```
The hypothesis of no correlation is rejected, even at the 1% level.
...
$\sigma_{\hat{\beta}}$ first proposed by @WHIT:80:
...
errors may share some common unobserved features of the regions. As an
example, in our 4 observations case, suppose that the first two
observations belong to one group, and the two other to an other group.
Then, a cluster-consistent estimator is obtained by summing the
...
the data set `urban_gradient` of @DURA:PUGA:20. It contains, the
...
and the regression line on @fig-dataug, the color of the points
...
In this example, the difference between the simple and the
heteroskedastic-robust estimation of the standard deviation of the slope
is small. However, we also have to investigate the potential correlation
...
of every covariates (the column of $X$) in deviation from the sample
...
\left(
\begin{array}{c}
\sum_{n=1} ^ N (\tilde{x}_{1n} - \bar{\tilde{x}}_1) \epsilon_n \\
\sum_{n=1} ^ N (\tilde{x}_{2n} - \bar{\tilde{x}}_2) \epsilon_n \\
\vdots \\
\sum_{n=1} ^ N (\tilde{x}_{1n} - \bar{\tilde{x}}_K) \epsilon_n
\end{array}
\right) 
= \sum_{n = 1} ^ N \psi_n
$$ 
...
vector of the first derivatives of the sum of squares residuals and is
...
\top \tilde{X} \mid X) \left(\frac{1}{N}\tilde{X} ^ \top \tilde{X})^{-1}\right) ^ {-1}
$$
...
If the errors are uncorrelated, but potentially heteroscedastic, we use
...
matrix computed for the single regression case. This estimator of the variance of the score can easily be obtained by defining the **estimating function** $F$ which is obtained by multiplying (elements by elements) the columns of the matrix of covariates by the vector of residuals:

$$
F =
\left(
\begin{array}{ccc}
\hat{\epsilon}_1 x_{11} & \ldots & \hat{\epsilon}_1 x_{1K} \\
\hat{\epsilon}_2 x_{21} & \ldots & \hat{\epsilon}_2 x_{2K} \\
\vdots & \ddots & \vdots\\
\hat{\epsilon}_N x_{N1} & \ldots & \hat{\epsilon}_N x_{NK}
\end{array}
\right)
$$
Then $F ^ \top F = \sum_{n=1} ^ N \hat{\epsilon}_n ^ 2 z_n z_n ^ \top$ is $N$ times the HC estimator of the meat and the HC estimator is then:
...
We go back to the electricity consumption estimation. `lm_elec` is the model fitted by OLS. We first compute the estimator manually, computing the meat (the crossproduct of the model matrix multiplied by the vector of residuals divided by the sample size) and the bread (the inverse of the crossproduct of the model matrix divided by the sample size). The covariance matrix is then computed using @eq-sandwich_hc_multi and we extract the standard deviations of the estimates, ie the square root of the diagonal elements of this matrix:
...
The **sandwich** package provides specialized function to construct the different pieces of the estimator, namely `estfun`, `meat` and `bread`:
...
`sandwich::vcovHC` uses thes functions to compute the HC estimator; it's first
...
heteroscedasciticy-robust estimator can be obtained by setting the
`type` to `"HC1"`, $\ldots$, `"HC5"` to perform some kind of degrees of
...
[^non_spherical-2]: See @ZEIL:06 for more details concerning the other
...
In this example, the robust standard errors are very close to the ones obtained using the simple formula, although the Breusch-Pagan test rejected the hypothesis of homoscedasticity. This is because the source of heteroscedasticity (the number of customers) is not a covariate of the model.
...
- `tapply` performs an operation (third argument) on a vector conditional (first argument) on the values of an other vector (second argument).
...
We can more simply use the `sandwich::vcovCL` function [@ZEIL:KOLL:NATH:20] to compute the clustered estimator. The clustering variable is defined using the `cluster` argument
...
Finally, `twins` being a pseudo-panel, the `plm` package can therefore be used. To OLS estimate can be fitted with `plm::plm` by setting the `model` argument to `"pooling"`.
...
## Generalized least square estimator {#sec-gls}
...
the generalized least square can then be used instead.
...
$$
\mbox{V}(\hat{\gamma}) = \mbox{E}\left[(Z^\top\Omega^{-1}Z) ^ {-1} Z^\top \Omega ^ {-1} \epsilon\epsilon^\top \Omega^{-1}Z(Z^\top \Omega ^ {-1} Z)^{-1}\right]=(Z^\top\Omega^{-1}Z)^{-1}
$$
...
-   the $\Omega$ matrix is a square matrix of dimension $N\times N$, and
    ...
    invert it using a computer for a large sample,
...
$C ^ \top C = \Omega ^ {-1}$. Then, @eq-glsmatrix can be rewriten,
denoting $w^*=Cw$:
...
[^non_spherical-3]: $(AB) ^ {-1} = B^{-1} A^{-1}$ if the inverse of the
    two matrix exists, see @GREE:18, online appendix p 1074.
...
squares residuals. The weight of each observation is $1 / \sigma_n ^ 2$. Therefore, an observation $n$ for which $\sigma_n^2$ is high will
...
that returns only positive values, $w$ is a set of variables and
$\delta$ a vector of parameters. If, for example $h$ is the
exponential function (which is a very common choice), the scedastic
function is: $\ln \sigma_n ^ 2 = \delta^\top w_n$ and the $\delta$
...
For the electricity consumption example, we know that the unconditional variance of $y$ in city $n$ is $\sigma_{yn} ^ 2 = \sigma_c ^ 2 / I_n$, $I_n$ being the number of consumption units in city $n$ and $\sigma_c ^ 2$ the variance of the individual consumption. Assuming that the same relation applies for the conditional variance, then $\sigma_{\epsilon n} ^ 2 = \sigma ^ 2 / I_n$ and, therefore, the weights ($1 / \sigma_{\epsilon_n}$) are proportional to $I_n$. The WLS estimator can then be obtained by computing OLS on series multiplied by $\sqrt{C_n}$:
...
The efficiency gain of using WLS is consequent, as the standard deviations of the estimates reduce by about 25/50% depending on the coefficient.
...
-   the number of observations for each entity (the number of
    ...
    transformation is close to a difference from the individual mean for
    large $T$.
...
[^non_spherical-4]: See also @SWAM:AROR:72, @AMEM:71 and @NERL:71

The residuals of the OLS regression were already added to the `twins` data set and 
$B \hat{\epsilon}$ was computed. We then comput $W\hat{\epsilon} = \hat{\epsilon} - B \hat{\epsilon}$ and we use @eq-varmxt and @eq-varidios to compute $\hat{\sigma}_\iota ^ 2$, $\hat{\sigma}_\nu ^ 2$, $\theta = 1 - \hat{\sigma}_\nu / \hat{\sigma}_\iota$ and then $\hat{\sigma}_\eta ^ 2 = (\hat{\sigma}_\iota ^ 2 - \hat{\sigma}_\nu^2) / T$.
...
The second specific part of the output gives information about the variances of the two components of the error. We can see here that the individual effects account for only 15% of the total variance of the error. Therefore, only a small part of the individual mean is removed while performing GLS (14.3%). This information can be obtained using the `ercomp` function:
...
The model can also be estimated by maximum likelihood:
...
```{r}
ols_sd <- vcovCL(lm_twins, ~ family, type = "HC0", cadjust = FALSE) %>% 
  stder
gls_sd <- vcov(gls_twins) %>% stder
rbind(ols = ord_sd, gls = gls_sd, ratio = gls_sd / ols_sd)
```
...
It seems here that GLS is much more efficient that OLS as the standard error of the slope is 4 times smaller.
...
decomposition, ie computing the upper-triangular matrix $C$ which is
...
More simply, the `systemfit::systemfit` function can be used, with the `method` argument set to `"SUR"`:
...
$$
Denote $B$ the matrix containing the coefficients $\beta_{ij}$. Remind that, imposing the homogoneity of degree one of the cost function, we imposed that $\beta_{iI} = - \sum_{i=1}^{I-1}$. Therefore $\beta_{iI}$ was not estimated and we must add it the $B$ matrix using this formula:
...
elasticity of the demand materials with the price of labor as $0.1039$
is the elasticity of the demand of labor with the price of materials.
The price elasticities indicates that the demand for the three inputs is
