...
`dutch_railways`^[Used by @BENA:BOLD:BRAD:93 and @MEIJ:ROUW:06.] is an example
  of a *wide* data set:
...
`toronto_montreal`,^[Used in particular by [@FORI:KOPP:93],
...
@BHAT:95 estimated the heteroskedastic logit model on the
...
To illustrate the estimation of the nested logit model, we use the `telephone` data set, used by @TRAI:MCFA:BENA:87 and @WALK:BENA:BOLD:07.
...
This so called GHK algorithm [@GEWE:KEAN:RUNK:94]  can be described as follow :
