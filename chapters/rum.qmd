...
Netherlands in 1987. Each individual has responded to several (up to 16)
...
only once in the data. It is cumbersome for alternative
...
respect to the covariates, which can be be choice situation-specific ($z_n$)
...
Consumers' surplus variations range from 0.6 to 31 Canadian \$, with a
...
Two supplementary coefficients (`sp.train` and `sp.air`) are estimated ($\theta_j$ in @eq-prob_heterosc_logit), the third one, for the reference modality being set to 1.
...
A more appealing approach consists on considering $\beta_n$ as
...
We obtain the value of `r round(mv[1], 1)` euros for an hour of traveling, `r round(mv[2], 1)` euros for a change and `r round(mv[3], 1)` euros to travel in a more comfortable class.^[Remind that the survey took place in 1987. The values should be multiplied by  1.73 to get euros of 2024.] We then estimate a model with three random parameters, `time`, `change` and
...
decomposition\index[general]{Choleski decomposition} of the covariance matrix of the three random parameters.
