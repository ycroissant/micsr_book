...
the **conditional expectation** of $y$: \index[general]{contional expectation|(}
...
is necesseraly supported. Natural estimators of the conditional expectations
...
This can be illustrated using a scatter plot (see @fig-birthwt)\idxdata{cigarettes}{micsr}, with
...
The expected wage for the three levels of education are $\mu_{xh} = 8$, $\mu_{xb} = 9.5$ and $\mu_{xg} = 10.5$. The expected wage is then $\mu_y = 0.4 \times 8 + 0.3 \times 9.5 + 0.3 \times 10.5 = 9.2$. We first compute the 1^st^ term of the covariance between wage and education for $x = x_h = 0$:
...
\index[general]{contional expectation|)}
\index[general]{covariance|)}

## The model and the data set {#sec-simple_model_data}
...
is reproduced from @BONN:04 p. 364-366\index[author]{Bonnel}.
...
We seek to draw a straight line that is closest as
...
## Geometry of least squares, variance decomposition and coefficient of determination {#sec-simple_geometry}
...
More generally, we denote $R$ the number of replications and we construct a data frame with $R \times N$ lines. The columns are the 13 values of $x$ (fixed in this simulation), the vector $\epsilon$ drawn in a normal distribution with a standard deviation equal to 0.08 and $y = \alpha + \beta x + \epsilon$, with $\alpha = 0.2$ and $\beta = 0.032$ as previously. Finally we add a variable `smpls` that is a vector containing integers from 1 to $R$ repeated 13 times that will be used to identify the sample:\idxfun{tibble}{tibble}
...
`id`. The `lm` function\idxfun{lm}{stats} has a `subset` argument that must be a logical expression used to select only a subset of the data frame. For example, to get the fitted model for the 1^st^ sample, the logical expression is `id == 1`  and one can use:
...
```
 ...
 Starting with `small_datas`, we use `tidy::nest`\idxfun{nest}{tidyr} to nest the data frame using the `id` variable (`.by = id`); the name of the results is by default `data`, but it can be customized using the `.key` argument:
...
\idxfun{nest}{tidyr}\idxfun{transmute}{dplyr}\idxfun{map}{purrr}\idxfun{summarise}{dplyr}\idxfun{sd}{statss
