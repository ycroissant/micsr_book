...
writing directly `pxt` is like writing `print(pxt)`\idxfun{print}{base}, the side effect
...
We can check that the residuals sums to 0 and are uncorrelated with the covariate and the fitted values:\idxfun{sum}{base}
...
`id`. The `lm` function\idxfun{lm}{stats} has a `subset` argument that must be a logical expression used to select only a subset of the data frame. For example, to get the fitted model for the 1st sample, the logical expression is `id == 1`  and one can use:
...
\idxfun{summarise}{dplyr}\idxfun{sd}{stats}\idxfun{mean}{base}
```{r}
...
\idxfun{tibble}{tibble}\idxfun{mean}{base}\idxfun{function}{base}
```{r}
#| label: ols_function
ols <- function(s){
  x <- s$x ; y <- s$y
...
\idxfun{nest}{tidyr}\idxfun{transmute}{dplyr}\idxfun{map}{purrr}\idxfun{summarise}{dplyr}\idxfun{sd}{stats}\idxfun{mean}{base}
