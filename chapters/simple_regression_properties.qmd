...
The variance of $\hat{\alpha}$ is the expected value of the square this expression; with the
hypothesis of homoskedasticiy and uncorrelation of the errors, it simplifies to:
...
Removing the expectation and dividing by the standard deviation, we get a standard normal deviate:
...
sample standard deviation $\hat{\sigma}_x ^ 2$:
...
df.residual(pxt)
```
$\hat{\sigma}_\epsilon$ is computed using:
...
point to the origin, as illustrated in @fig-indslopes, and to estimate $\beta$ by the mean of the $N$ slopes. Formally, we
have:
...
the arithmetic mean, $\sigma_{\tilde{\beta}} > \sigma_{\hat{\beta}}$ and therefore $\tilde{\beta}$ is less precise than
$\hat{\beta}$.
...
propriety of unbiasedness, even if we often encounter estimators which
...
where $\bar{x}$, $\hat{\sigma}_x ^ 2$ and $\hat{\sigma}_{x\epsilon}$ are the sample estimates of the population mean and variance of $x$ and of the covariance between $x$ and $\epsilon$. As sample size increases,
...
When $N\rightarrow +\infty$, $\hat{\beta}_N$ has a degenarate distribution, as it converges to a constant (which is $\beta$ if the estimator is consistent) as its variance tends to 0. In this subsection, we analyse the shape of the distribution of $\hat{\beta}$ as the sample size grows. We therefore need to consider a transformation of $\hat{\beta}_N$ which has a constant variance, and we'll see that it is $\sqrt{N}(\hat{\beta} - \beta)$. Starting again with the equation that relates $\hat{\beta}_N$ to the errors:

$$
\hat{\beta}_N = \beta + \sum_{n=1}^N c_n \epsilon_n = \beta + \frac{\sum_{n=1}^N \omega_n \epsilon_n}{\sqrt{N}\hat{\sigma}_x}
$$

with $\omega_n = \frac{x_n - \bar{x}}{\sqrt{N}\hat{\sigma}_x}$. Note that $\omega_n$ sums to 0 (as $c_n$), but that $\sum_{n=1}^ N\omega_n ^ 2=1$.


subtracting $\beta$ and multiplying by $\sqrt{N}$, we get:
...
4th moments equal 0 and 3, which are the corresponding
...
- highly asymetric, ie it has a long tail on the right side of the distribution, and a
...
Now we generate a large number $R$ of samples
...
distribution changes when the sample size increase. We can see more precisely on this figure the strength of the central limit
...
where $\stackrel{a}{\sim}$ means that the normal distribution is asymptotic and is very good approximation if the sample size is large enough (which is the case in general in microeconoetrics studies), whatever the distribution of the errors. 
...
$$
p = 2 \left[1 -\Phi\left(\left| \frac{\hat{\beta}-\beta}{\sigma_{\hat{\beta}}} \right|\right)\right]
$$
...
function. The result is an object with further details about the linear model
...
$\beta_0=0$. Finally, the last column contains the probability value.
...
The structural parameters are easily retrieved using @eq-structpar. The so-called Delta method can be used to compute their standard deviations. Denoting $f$ the first derivatives of $F$, we write a first order Taylor extension for $F^a$ and $F^b$:
...
So that the variance of the fitted structural parameters is:
...
($\hat{\alpha}, \hat{\beta}$) we have, for the price-time model:
...
which increase with the deviation of $x_n$ from the sample
mean. Moreover, as $\sigma_{\hat{y}}$ tends to 0 when $N$ tends to
infinity, $\hat{y}_n$ is a consistent estimator of $\mbox{E}(y
\mid x = x_n)$.

Consider now the standard deviation of $y_n = \mbox{E}(y \mid x = x_n) + \epsilon_n$. To the variation due to the estimation of $\alpha$ and $\beta$, we have to add the one associated to $\epsilon_n$. Therefore, the variance of $y_n$ is the sum of $\sigma_{\hat{y}_n}$ and $\sigma_\epsilon^n$ and therefore its standard deviation is:
...
A **confidence interval** for $\hat{y}_n$ is obtained by adding and substracting
from the point estimator the estimated standard deviation $\sigma_{\hat{y}_n}$ times the critical value (here a Student $t$ with $N-2=7$ degrees of freedom).
...
- for the confidence interval, we use `geom_smooth` with the default
...
prd
```
The model predicts that train's share increase from `r round(pull(prd, fit)[1], 3)` to `r round(pull(prd, fit)[2], 3)`, but the confidence intervals are quite large and overlaps. Present and predicted market shares are represented in red and blue on @fig-bordeaux, along with the confidence interval.
