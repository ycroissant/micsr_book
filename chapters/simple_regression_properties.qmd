...
To analyze the statistical properties of the OLS estimator, we use the
formula obtained in the previous chapter that indicates that the
difference between the estimated slope and the true value is a linear
...
-   the first one are called **exact** properties, and they apply
    whatever the size of the sample is,
...
problem in econometrics called **identification** that we'll encounter
...
that indicates the conditional expectation of wage as points for men
...
-   with a positive correlation, the OLS estimator is upwarded biased.
...
standard deviation of $\hat{\beta}$ is high (or that the estimator is
imprecise).
...
All these results can be illustrated by simulations. We start with our
reference case ($\sigma_\epsilon = 0.08$, $N = 9$ and $x$ is the
...
Note that we used twice the `dplyr::summarise` function. The first time,
it is used with `dplyr::group_by` so that a tibble with `R` lines is
...
We first compute "by hand" the standard errors of the OLS estimator
and we then use the relevant methods for `lm` objects to do so. We first
...
(which is, in the simple linear model, $N-2$), we have:
...
next take the square roots of the values:
...
is 0. To fit the model that impose this hypotheses, we need to fit the
same model without intercept. In **R**, this is performed using either
`- 1` or `+ 0` in the formula :
...
of this alternative estimator can be computed as follow:
...
arithmetic and the harmonic means of $x ^ 2$:
...
squares implies convergence in probability.^[See for example @AMEM:85, pages 89-90.] We'll also say in this case
...
-   unbiasedness is an exact property (true or false whatever the sample
    ...
    doesn't say anything about its variance,
...
[^simple_regression_properties-2]: These two estimators are inspired by
    @DAVI:MACK:04, page 97 and @DAVI:MACK:93, page 123-124.

$$
\dot{x}_N = \frac{1}{N - 1} \sum_{n=1}^N x_n
$$

Its first two moments can easily obtained by writing $\dot{x}_N$ as a
...
[^simple_regression_properties-3]: Inspired by @DAVI:MACK:93, pages
    126-127.
...
the shape parameters of $x_n$ ($\mu_3$ and $\mu_4$) and its 3rd and 4th
...
a standardized chi-squared distribution with one degree of freedom.
...
sample sizes; we use the "repeated in fixed sample" hypothesis^[See @DAVI:MACK:93, pp.116-117 for details.], ie we
...
returned if the `lower.tail` argument is set to `FALSE`:
...
$\frac{\hat{\beta}_N-\beta_0}{\sigma_{\hat{\beta}_N}}$ is a draw in a
...
Probability critics are computed using the `pnorm` function, which takes
...
**R** has different functions that extract informations about the
statistical properties of the fitted model. To illustrate their use, we
use once again the price-time model:
...
-   their standard error ($\dot{\sigma}_{\hat{\beta}}$),
...
corresponding covariate. If the hypothesis that $\beta_0$ is rejected,
...
small sample, it is worth considering the critical value for a Student
...
`r round(confint(pxt, level = .9)[2, 2], 3)`.
...
parameters, which are the lower and higher values of the travel time $a$
and $b$:
...
and $\hat{\beta}$ given in @eq-covariance_gamma:
...
between `r round(maxvt[1], 1)` and `r round(maxvt[2], 1)` euros.
...
Using @eq-covariance_gamma, we finally get:
...
which increases with the deviation of $x_n$ from the sample mean.
Moreover, as $\sigma_{\hat{y}}$ tends to 0 when $N$ tends to infinity,
...
limits of the confidence / prediction intervals:
...
`interval` argument set to `"confidence"` or `"prediction"`:
...
-   for the confidence interval, we use `geom_smooth` with
    ...
    in this case, we have a Gray zone which figure the confidence
...
features of the Paris-Bordeaux trip and the second one the new features.

```{r }
#| label : initial_bordeaux
initial <- filter(prtime, town == "Bordeaux") %>% 
...
is the `bordeaux` table in our example):
