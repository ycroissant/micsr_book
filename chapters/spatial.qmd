...
\idxfun{st\_geometry}{sf}\idxfun{plot}{base}
...
of China for the **countries** package, but was considered as a sovereign country in @ERTU:KOCH:07's\index[author]{Ertur}\index[author]{Koch} study. We then correct the code for the D.R. of Congo, we add Hong Kong using the `include` argument and we remove `Antarctica` with the exclude argument:
...
\idxfun{plot}{base}\idxfun{scale\_size\_continuous}{ggplot2}\idxfun{theme}{ggplot2}\idxfun{guides}{ggplot2}\idxfun{labs}{ggplot2}\idxfun{guide\_legend}{ggplot2}
...
An interesting feature of the model of @ERTU:KOCH:07\index[author]{Ertur}\index[author]{Koch} is that their specification enables the identification of all the structural parameters of the model, $\kappa$, $\phi$ and $\gamma$. If $\phi = 0$, $\theta = \gamma \beta$ and $\rho_y = \gamma$, so that: $y = \alpha + (I - \gamma W)X\beta + \gamma y + \epsilon$, the reduced form of the model is, in matrix form:
...
\idxfun{diag}{base}\idxfun{pnorm}{stats}\idxfun{abs}{base}\idxfun{cbind}{base}
