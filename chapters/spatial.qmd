...
We use the convention that the distance is positive for Texan's counties and negative for neighbouring states.
...
growth of the population and the logarithm of its initial value and a fitting line with a second degree polynomial:
...
The second example is from @ERTU:KOCH:07 who estimated a growth model for
...
- the annual growth rate, the difference of the logs of the gdp for
  1995 and 1960 divided by 35 (`growth`).
...
countries of `sp_solow` are present in the `countries` object, with the `check_join` function; the `by` argument indicates the series in `sp_solow` that identified the country:
...
related to the initial (1960) gdp. `countries` provides a `plot` method which draws a basic map using **ggplot**. A basic thematic map can be drawn using the `fill` argument to fill the areas of the countries and the `centroid` or the `capital` arguments to draw a point at the position of the centroid or the capital of each country. Any **ggplot** functions can be used to customize the graphic:
...
These two operations of defining the set of neighbors and their weights are performed by the **spdep** package [@PEBE:BIVA:23].
...
polygons which have only one common point are neighboors. Setting `queen`
...
$$
y = \alpha i + X \beta + \epsilon \; \mbox{with} \; \epsilon = \lambda W \epsilon + \eta
$$ {#eq-sem}
...
covariance matrix of the coefficients ($\sigma ^ 2 (\tilde{X}^\top \tilde{X}) ^ {-1}$)
...
\alpha + \beta ^ \top x_n + \rho \tilde{y}_n + \epsilon_n$ or, in matrix
form:

$$
y = \alpha i + X \beta + \rho W y + \epsilon = Z \gamma + \rho W y + \epsilon
$$
The reduced form of the model is:

$$
y = (I - \rho W) ^ {-1} Z \gamma + (I - \rho W) ^ {-1}\epsilon
$$  {#eq-sar}
...
$$
France has two neighbors (Italy and Spain) and Italy and Spain only one (France). The weights are such that they sum to 1 for each line. Consider a variation of the unique covariate in Spain ($\Delta x_3$) and denote $\beta$ the corresponding coefficient. The direct effect on the response for the three countries is obviously $\Delta y_0 ^ \top = (0, 0, \beta \Delta x_3)$. This increase of $y$ in Spain implies an increase of $y$ in the neighboring country, France. Therefore, $\Delta y_1 ^ \top = (0.5 \rho \beta \Delta x_3, 0, 0)$. This increase of $y$ in France implies an increase of $y$ in the neighboring countries, Italy and Spain: $\Delta y_2 ^ \top = (0, 0.5 \rho ^ 2 \beta \Delta x_3, 0.5 \rho ^ 2 \beta \Delta x_3)$. This increase of $y$ in Italy and Spain implies an increase of $y$ in the neighboring country of Italy and Spain, which is France: $\Delta y_3 ^ \top = (0.5\rho ^ 3 \beta \Delta x_3, 0, 0)$, etc. To take a numerical example, consider $\beta \Delta x_3 = 1$ and $\rho = 0.3$. Then, $\Delta y_0 ^ \top = (0, 0, 1)$, $\Delta y_1 ^ \top = (0.15, 0, 0)$, $\Delta y_2 ^ \top = (0, 0.045, 0.045)$ and $\Delta y_3 ^ \top = (0.0135, 0, 0)$. In total, we have $\sum_{i = 0} ^ 3 \Delta y_i ^ \top = (0.177, 0.045, 1.045)$. These 4 effects and their sum is computed below:
...
The total effect of $\Delta x_3$ is obtained using the formula: $\Delta y = (I - \rho W) ^ {-1} \Delta x$:
...
$$
\eta = (I - \lambda W) y - (I - \lambda W) Z\gamma
$$ {#eq-etasem}
...
$$
y = \rho W y + Z\gamma +   W Z\theta + \epsilon
$$

with $\epsilon$ iid errors. The **common factor** hypothesis is that $\theta = - \rho \gamma$. In this case, we have:

$$
(I - \rho W) y =  (I - \rho W) Z\gamma + \epsilon
$$
Or finally:

$$
y =  Z\gamma + (I - \rho W) ^{-1} \epsilon
$$
which is the SEM model, as denoting $\nu = (I -\rho W) ^ {-1} \epsilon$ the errors of this model, we have $\nu = \rho W \nu + \epsilon$. 

Once it has been established that there is some spatial dependence, one has to choose the "right" specification. Several tests have been proposed, based on the three tests principle. Lagrange multiplier tests proposed by @ANSE:BERA:FLOR:YOON:96, are popular as they only require the OLS estimation. Different flavors of tests have been proposed, testing $\rho = 0$, $\lambda = 0$ or $\rho = \lambda = 0$. The basic version of, for example the first test ($\mbox{H}_{0}: \rho = 0$) has, as a maintained hypothesis that $\lambda = 0$. A robust version of the test has been proposed which doesn't impose this maintained hypothesis. The common factor hypothesis can easily be tested using a likelihood ratio. The Wald statistic is less easy to compute, has a set of non-linear hypothesis $\theta = - \rho \beta$ is tested.

### Application to the growth model

To overcome the irrelevant empirical results of the standard Solow model, @ERTU:KOCH:07 consider an endogenous growth model with spillovers. The production function is as usual a Cobb-Douglas:
...
$$
where $k_n(t) = K_n(t) / L_n(t)$ is the physical capital per worker, $\Omega(0)$ is the initial level of the technology and $\mu$ is a constant rate of technological growth, as in the Solow model. The next term takes into account the spillover effect of domestic investment, following @ROME:86 and the strength of this spillover effect is measured by $\phi$. The last term is specific to the model of @ERTU:KOCH:07, for which the spillovers are not restricted to domestic investment, but concerns also the technology of neighboring countries. The effect of the technology of country $m$ on the technology of country $n$ is the product of a constant parameter $\gamma$ and a specific weight, $w_{nm}$, which is a decreasing function of the distance between both countries. Denoting $v = n + g + \delta$, with $n$ and $g$ the growth rates of the labor force and of the technological progress and $\delta$ the depreciation rate, 
@ERTU:KOCH:07 showed (equation 10, page 1038) that, at the steady state, the output per capita is:
...
&-& \frac{\gamma\kappa}{1 - \kappa - \phi} \sum_{m\neq n}^ N w_{nm} \ln s_m + \frac{\gamma\kappa}{1 - \kappa - \phi} \sum_{m\neq n}^ N w_{nm} \ln v_n \\
&+& \frac{\gamma(1 - \kappa)}{1 - \kappa - \phi} \sum_{m\neq n}^ N w_{nm} \ln y_m ^ *
\end{array}
$$
In matrix form, denoting $y = \ln y^*$, $X = (\ln s \ln v)$, $\beta = \left(\begin{array}{c} (\kappa + \phi) / (1 - \kappa - \phi)\\ - (\kappa + \phi) / (1 - \kappa - \phi)\end{array}\right)$, $\theta = \left(\begin{array}{c} \gamma \kappa / (1 - \kappa - \phi)\\ - \gamma  \kappa  / (1 - \kappa - \phi)\end{array}\right)$ and $\delta = \gamma (1 - \kappa) / (1 - \kappa - \phi)$, the model can be written as:

$$
y = \alpha i + X \beta + WX \theta + \delta Wy + \epsilon
$$

if $\gamma = 0$, the model reduces to the model of @ROME:86 and the last two terms disappear. Note that in this case $\kappa$ and $\phi$ are not identified, but only their sum. Moreover, if $\phi$ is also 0, we get the Solow model. Wheter $\phi$ equals zero or not, the resulting model is a standard linear model already estimated in @sec-solow_model:

```{r}
ols_solow <- lm(log(gdp95) ~ lns + lnngd, sp_solow2)
```

Moreover, imposing the theoretical constraint of the growth model, we must have the sum of the two coefficients of `lns` and `lnngd` equal to 0. Therefore, the constrained model can be estimated using the difference of the two covariates as the unique regressor. For convenience, we call this sum `lns` and we store it in a new data frame called `sp_solow3`. The constrained OLS model can then be estimated:

```{r}
sp_solow3 <- sp_solow2 %>% mutate(lns = lns - lnngd)
ols_solowc <- lm(log(gdp95) ~ lns, sp_solow3)
```
...
library(lmtest)
library(car)
wald_stat <- function(x, R, q = NULL){
  if (is.null(q)) q <- rep(0, nrow(R))
  if (ncol(R) != length(coef(x))) 
    stop("The number of columns of R should equal the number of coefficients")
  .coefs <- R %*% coef(x)
  .vcov <- R %*% vcov(x) %*% t(R)
  drop(crossprod(.coefs, solve(.vcov, .coefs)))
}
sp_solow4 <- sp_solow2 %>% mutate(lns2 = lns + lnngd)
lm_solow <- lm(log(gdp95) ~ lns2 + lnngd, sp_solow4)
lm_solowc <- lm(log(gdp95) ~ lns2, sp_solow4)
loglm_solow <- loglm(gdp95 ~ lns + lnngd, sp_solow2)
loglm_solowc <- loglm(gdp95 ~ lns, sp_solow3)
```

```{r}
#| collapse: true
linearHypothesis(ols_solow, "lns + lnngd = 0", test = "Chisq") %>% sight
lrtest(ols_solow, ols_solowc) %>% sight
linearHypothesis(m1, c("lns + lnngd = 0", 
                       "lag.lns + lag.lnngd = 0")) %>% sight
lrtest(m1, m1c) %>% sight
```
...
An interesting feature of the model of @ERTU:KOCH:07 is that their specification enables the identification of all the structural parameters of the model, $\kappa$, $\phi$ and $\gamma$. If $\phi = 0$, $\theta = \gamma \beta$ and $\delta = \gamma$, so that: $y = \alpha + (I - \gamma W)X\beta + \gamma y + \epsilon$, the reduced form of the model is, in matrix form:
...
The value of $\kappa$ is compatible with the observable share of the profits in the national product. $\phi$ is significant only at the 10% level. Finally, $\gamma$ is highly significant, which is a confirmation of the relevance of the model of @ERTU:KOCH:07.
