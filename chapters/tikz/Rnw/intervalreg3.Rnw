...
matptsup <- cbind(seq(thr2, xmax, 0.01), dlnorm(seq(thr2, xmax, 0.01)))
matptsup <- rbind(c(thr2, 0), matptsup, c(xmax, dlnorm(xmax)), c(xmax, 0))
...
TIKnode(c(thr2, 0), "$\\mu_l$", pos = "below")

TIKnode(c((thr2 + xmax) / 2, 0.01), "$\\mbox{P}(y > \\mu_l)$", pos = "above")
