...
matptsup <- cbind(seq(xmin, 0, 0.01), dnorm(seq(xmin, 0, 0.01), 0.5))
...
TIKline(list(c(0.5, ymin), c(0.5, ymax)), options = "->")
TIKline(list(c(0, 0), c(0, dnorm(0, mu1) / (1 - PI))))#, options = "dotted")
TIKnode(c(xmax, 0), "$\\epsilon$", pos = "right")
#TIKnode(c(0, ymax), "y", pos = "above")
TIKnode(c(0, 0), "$-\\beta'x$", pos = "below")
TIKnode(c(mu1, 0), "$0$", pos = "below")
TIKnode(c(3.2, 0.3), "$\\frac{1}{\\sigma}\\phi\\left(\\frac{\\epsilon}{\\sigma}\\right)$")
TIKline(list(c(2, 0.29), c(1.5, dnorm(1.5, mu1))), options = "->") 
TIKnode(c(3.2, 0.5), "$\\frac{1}{\\sigma}\\frac{\\phi\\left(\\frac{\\epsilon}{\\sigma}\\right)}{\\Phi\\left(\\frac{\\beta'x}{\\sigma}\\right)}$")
...
#TIKcoords(c(mu1, dnorm(mu1, mean = mu1, sd = sigma)), labx = "$0$")
#TIKnode(c(mu1, 0), mu1, pos = "below")
