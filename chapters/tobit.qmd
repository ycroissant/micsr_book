...
its utility subject to its budget constraint, which writes $x=p_y q_y +
...
to the labor force: the response is the wage offered to women, but it is only observed for women who participate to the labor market.
...
On the contrary, samples that consist on individual surveyed in a
...
suppose that $\epsilon_n \sim \mathcal{N} (0, \sigma_\epsilon^2)$, ie that the errors of the
...
not for a truncated sample for which all the values of $y$ are
positive.
...
$y_n - \hat{\gamma} ^ \top z_n -  \hat{\sigma} \hat{r}_n$.
The second matrix, that uses the covariance matrix of the first stage probit regression, takes into account the fact that $\hat{r}_n$ is introduced in place of $r_n$.
...
This **symmetrically truncated least square** estimator easily extends to the case of
...
@WILH:08\index[author]{Wilhelm} considers the value of the donation in logs and subtract from it $\ln
...
- the @MATS:SHER:06\index[author]{Sherlund}\index[author]{Matschke}'s "short" model, that imposes that $\beta_2 = \beta_3$,
...
The relative merits of the two-parts and the heckit models have been discussed in numerous articles, especially in the field of health economics. The arguments are presented in @JONE:00\index[author]{Jones} and @DOW:NORT:03\index[author]{Dow}\index[author]{Norton}.
...
We start by defining the selection and the outcome equations. The authors suppose that the probability of receiving a fine depends on the difference between the actual and the limit speed, the ethnicity, the sex and the age of the offender, the fact that the driving license is commercial and covariates describing local fiscal conditions and place of residence of the offender in interaction with the dummy for state officers. As the log of age is introduced in interaction with the `female` dummy, we divide the age by the sample mean so that the coefficient of `female` is the effect for the mean age.
