...
by @WOOL:10, pp.517-520, 0 is a relevant value for the variable (this is not the case for the situation described in the next section), but all the values of 0 don't
...
process takes the **top-coding** form and $13030$ is not a relevant value of the covariate.
...
econometrics [@TOBI:58] and was proposed by @GOLD:64 because of its similarity
with the probit model.
Estimation on truncated samples leads to the **truncated regression
model** [@CRAG:71; and @HAUS:WISE:76; @HAUS:WISE:77].
...
- the conditional expectation of $y$ is now longer equal to
  $\mu_n = \alpha+\beta ^ \top x_n$, but to $\mu_n + \sigma r(\mu_n / \sigma)$ or, stated differently, the errors of the model are
  correlated with the covariate as $\mbox{E}(\epsilon \mid x) = \sigma r(\mu_n / \sigma)$.
...
least squares, probit and two-steps estimators). We'll then present
...
or, following @WHIT:80's argument, the square of the residual
$y_n - \hat{\gamma} ^ \top z_n -  r(\hat{\delta} ^ \top z_n)$.
The second matrix, that use the covariance matrix of the first stage probit regression, takes into account the fact that $\hat{r}_n$ is
...
to @OLSE:78: $\delta = \gamma / \sigma$ and $\theta = 1 / \sigma$.
@OLSE:78 showed that the log-likelihood function of the censored model
...
homoskedastic, the semi-parametric estimator presented in this section [@POWE:86]
...
@WILH:08 consider the value of the donation in logs and subtract from it $\ln
...
Using `micsr::tobit1`, we also estimate the two-steps, 
...
first two columns of table 3 of @WILH:08, page 577. Note that the OLS
...
parameters to be estimated. The `crch::crch` function [@MESS:MAYR:ZEIL:WILK:14] enables the estimation of
...
Conditional moments tests can be computed using
...
where, as usual, $d_n = \mathbf{1}(y_n ^ * > 0)$. Note that, as the value of the response is partly observed, $\sigma_\epsilon$ is identified, contrary to the probit case where it has been arbitrarily set to 1. As for the probit model, several estimators are available: the maximum likelihood, the two-steps and the minimum $\chi ^ 2$ estimators. Moreover, the hypothesis of exogeneity can be performed by computing a Wald test on the two-steps estimator.

In the protection for sale model of @GROS:HELP:94, empirically tested by @GOLD:MAGG:99, trade protection is determined by capital owners' lobbying. @MATS:SHER:06 extend this model by introducing the potential effect of trade unions' lobbying on trade protection. The response is non-tariff barriers coverage ratio $\tau$, transformed as $\frac{\tau}{1 + \tau}$. The two covariates of @GROS:HELP:94 are the inverse of the import penetration ratio^[The value of gross imports divided by the value of shipments.] divided the importation demand elasticity $x_1$ and this variable in interaction with a dummy for capital owner's lobbying $x_2$. The supplementary covariate in @MATS:SHER:06's model $x_3$ takes into account trade union's lobbying. The data set is called `trade_protection`. We first compute the response and the covariates using raw series:^[The third covariate is quite complicated to compute and is directly available  in `trade_protection` as `labvar`.]
...
@MATS:SHER:06's theoretical model result in the following structural equation:

$$
\frac{\tau}{1+\tau} = - \frac{\Theta}{\Theta + a} x_1 + \frac{1}{\Theta + a}x_2 + \frac{1}{\Theta + a} x_3
$$
Therefore, denoting $(\alpha, \beta_1, \beta_2, \beta_3)$ the reduced form parameters, @MATS:SHER:06's model implies that $\alpha = 0$ and that $\beta_2 = \beta_3$ as, in @GROS:HELP:94's model $\alpha = \beta_3 = 0$. @MATS:SHER:06 suspect that the three covariates may be endogenous and therefore use the @SMIT:BLUN:86's model. Three models are estimated: 

- the @GROS:HELP:94's model (GH in short), ie $x_3$ is omitted,
- the @MATS:SHER:06's "full" model, that doesn't impose that $\beta_2 = \beta_3$,
- the @MATS:SHER:06's "short" model, that impose that $\beta_2 = \beta_3$,

The hypothesis that $\alpha = 0$ is not imposed in the three models.
Numerous instruments are used, as described in @MATS:SHER:06, page 415 and are defined below as a one-side formula:
...
The results are presented in @tbl-protection_for_sale.^[See @MATS:SHER:06, table 3, page 417.] As in the original article, we use the name of the coefficients and not the name of the terms, which are listed below:
...
endogtest(Formula::as.Formula(y ~ x1 + x2 + x3, inst), 
          trade_protection, model = "tobit") %>% gaze
```
The exogeneity hypothesis is not rejected for the GH model, but it is for the two versions of @MATS:SHER:06's model. The GH model, which is a special case of the Full model is relevant if the hypothesis $\beta_3 = \gamma_3 = 0$ are not rejected. Performing a Wald test, we get:
...
and the hypothesis is therefore rejected at the 5% (but not at the 1%) level. Then, the hypothesis that $\beta_2 = \beta_3$ and $\gamma_2 = \gamma_3$ implied by @MATS:SHER:06's model can be tested:
...
response.
The seminal papers about the tobit-2 model are @GRON:73 with his model
of female labor supply and Heckman [-@HECK:76; -@HECK:79] who precisely described the
statistical properties of this model and proposed a two-steps
estimator.
Hurdle models, proposed by @CRAG:71 can also be considered as tobit-2
...
two-steps models the fact that the two equations can be estimated
...
Hurdle models can be estimated using the **mhurdle** package [@CARL:CROI:23].
...
is a two-steps estimator, sometimes called the **heckit** estimator. It
consists on first regressing $\mathbf{1}(y_1^* > 0)$ on $x_1$ using a probit, and then estimating $r(\gamma^\top z_1)$ by
$r(\hat{\gamma}^\top z_1)$, $\hat{\gamma}^\top z_1$ being the linear
...
The relative merits of the two-parts and the Heckit models have been discussed in a lot of articles, especially in the field of health economics. The arguments are presented in @JONE:00 and @DOW:NORT:03.

### Application

@MAKO:STRA:09 analyse the behavior of policemen in terms of fees
