...
4 squares and named by the two digits (the observed values of $w$ and $d$) indicated in a gray square. The potential counterfactual are represented by two circles inside the square.
...
on $y$ in @eq-fourcats is zero because the causal effect of $w$ on $d$ is zero. The monotonicity assumption rules out
...
treatment effect** (**LATE** in short) to stress the fact that what is estimated
...
are not the same. This is the case in @HONG:13's article, who
...
The matching estimator was first proposed by @ROSE:RUBI:83 and the interest in matching methods in econometrics really started with   @DEHE:WAHB:99 and @DEHE:WAHB:02. These two articles revisited the results of @LALO:86 who showed that the estimation of treatment effect with observational data can be 
...
As an example, we replicate the results of @ICHI:MEAL:NANN:08 who studied
...
Finally, `step = "atet"` gives the values of the estimated ATET (average treatment effect on the treated) and different estimates of its standard deviation:
...
The ATET (`r round(atet, 4)`) is close to the one obtained previously using the algorithm based on stratas (`r round(unname(summary(ps)$atet[1]))`).
