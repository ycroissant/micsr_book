...
and an alternative estimator is obtained using the difference between the mean values of the outcome in the treatment and the control group:
...
To overcome these difficulties, different estimators have been suggested for observational data, which rely on different assumptions that are detailed, for example, in @CAME:TRIV:05, pp. 862-865.
...
individuals that are drawn from the same population and therefore the average observable and unobservable characteristics in
...
$$
\hat{\sigma}_x ^ 2 = \frac{\sum_{n\in T} (x_n - \bar{x}_n) ^ 2 + \sum_{n\in C} (x_n - \bar{x}_n) ^ 2}{N_T + N_C - 2}
$$
...
#| collapse: true
t.test(distance ~ group, afghan_girls) %>% sight
```
...
t.test(distance ~ group, afghan_girls, var.equal = TRUE) %>% sight
```
...
lm(distance ~ group, afghan_girls) %>% sight
```

The coefficient of the intercept is the mean for the control group and the coefficient of `grouptreatment` is the difference of the means for the two groups. The t-value is excactly the one obtained using `t.test` with `var.equal = TRUE`.
...
Denoting $o$ and $e$ the observed and hypothesized frequencies, $i$ is the index of the cell and $N$ the size of the
sample:
...
In the `afghan_girls` data set, `ethny` is a factor with levels `"other"`, `"farsi"` and `"tajik"`. We first compute the frequency table using the `table` function, which is similar to `dplyr::count`, but the input is a series and not a data frame and the result is an object of class `table`, which is much alike an object of class `matrix`, and not a data frame. We then compute the relative frequencies using `prop.table` function. Note that `prop.table` has a `margin` argument that we don't use here. ^[If `margin = 1`, the frequencies sum to one for each line and if `margin = 2`, they sum to one for each column.]
...
In case of independence, the frequencies are using the outer product of the marginal frequencies:
...
This test can be conveniently computed using the `chisq.test` function:

```{r }
#| label: pearson_chisq_test
#| collapse: true
chisq.test(table(afghan_girls$group, afghan_girls$ethny)) %>% sight
```

and the hypothesis of independence is not rejected. The whole table of balance is often called "Table 1", as it is the first table that appears in most articles in medical reviews. It 
can be computed using the `gtsummary::tbl_summary` function, which has a `by` argument that indicates the grouping variable.  `tbl_summary` has a lot of arguments and auxiliary function that enables to customize the results. ^[Detailed documentation of the package can be found on the web site of the package <https://www.danieldsjoberg.com/gtsummary/> and on @SJOB:21]. The balance table for the `afghan_girls` data set is
...
t.test(test ~ group, afghan_girls, subset = sex == "girl") %>% sight
```
...
binary and the instrument $z$ is also binary. 
The value of the outcome for individual $n$ for a given combination of
$d$ and $z$ is denoted: $y_n(z_n, d_n)$. $z$ can be used as an
...
variable litterature. Therefore, potential outcome can be defined as a
function of the treatment variable only: $y_n(z_n, d_n) = y_n(d_n)$
...
The last assumption is **monotonicity** [@IMBE:ANGR:94] which states
...
level. 4 cathegories can be considered (represented on @fig-ivset):
...
4 squares and named by the two digits number (the observed values of $z$ and $d$) indicated in a grey square. The potential conterfactual are represented by two circles inside the square.
For example the square called $00$ contains the individuals
for which $z=0$ and $d=0$. For those individuals the unobserved
conterfactual is $z=1$ and $d$ either equal to 0 or 1. If the conterfactual is $z = 1, d = 0$, the individual is a never-taken, $d= 0$ whatever the value of $z$. If the conterfactual is $z=1, d=1$, the individual is a complier: with $z=0$, $d=0$, but a change of $z$ from 0 to 1 results in a change of $z$ from 0 to 1.
With the always takers and the never takers, the causal effect of $z$
on $y$ in @eq-fourcats is zero because the causal effect of $z$ on $d$ is zero. The monotoniciy assumption rules out
...
general case, the following population estimand: $\frac{\mbox{cov}(y, z)}{\mbox{cov}(d, z)}$, 
...
Treatment Effect (**LATE**) to stress the fact that what is estimated
...
those for which a change in the value of $z$ results in a change in the
value of $d$.
The numerator is also called the **reduced form** equation, as it measures the effect of the instrument on the outcome. The denominator called the **intention to treat** equation, it measures the effect of the instrument on the treatment dummy.

As an example, we use the data set of 
@ANGR:BETT:BLOO:KING:KREM:02 called `paces`. The authors investigate the effect of a large
...
#| label: ols_paces
#| collapse: true
lm(educyrs ~ privsch, paces) %>% sight
```
...
#| collapse: true
lm(privsch ~ voucher, paces) %>% sight(coef = 2)
lm(educyrs ~ voucher, paces) %>% sight(coef = 2)
```
...
`phone` is a dummy for owning a phone, `age` and `sex` are the age ond the sex of the
and `strata` is a strata of residence. We
...
observable variable (called the **forcing variable**), and more precisely, on the fact that the value of
...
except that the first receives the treatment and the second doesn't. This is called a **regression discontinuity** (**RD** in short) design.
...
The first one, called `probation`, is from @LIND:SAND:OREO:10 who studied the effects of academic probation on academic performance in Canadian universities. If a student's grade point average
...
reporting.^[see @LIND:SAND:OREO:10, footnote 20 page 104.] We remove these misleading observations:
...
@BUSE:15 analyzed the effect of income on religiousness. He considered 
...
were collected in 2011 and are available as the `bdh` data set. The
...
the two regression lines. It is about 0.25 points of gpa in our example and
...
made while estimating the effect of the treatment: the **bandwidth**, ie the range of values of the forcing variable used in the estimation and the  degree of smoothness of the fitting line.
A larger bandwidth leads to a more precise estimator (as the number of
...
#| collapse: true
lm(gpa2 ~ distcut * probation, probation,
   subset = abs(distcut) < 0.5) %>%  sight(coef = "probationyes")
lm(gpa2 ~ (distcut + I(distcut ^ 2)) * probation, probation,
   subset = abs(distcut) < 1.5) %>%  sight(coef = "probationyes")
lm(gpa2 ~ (distcut + I(distcut ^ 2)) * probation, probation,
   subset = abs(distcut) < 0.5) %>%  sight(coef = "probationyes")
```
...
#| warning: false
with(probation, rdrobust(gpa2, distcut)) %>% sight
```
...
ivreg::ivreg(attendmonth ~ selben * treated | selben * eligible, 
             data = bdh) %>% sight(coef = "treated")
```

The effect of receiving the grant increases by 1.72 monthly church attendance and the effect is significant. The `rdrobust` function can be used the effect in a fuzzy discontinuity context using the `fuzzy` argument to indicate the treatment variable. A matrix of supplementary covariates can be added using the `covs` argument.

```{r }
#| label: bdh_rdrobust_te
with(bdh, rdrobust(y = attendmonth, x = selben, fuzzy = treated,
                   covs = cbind(age, hsize, schooling))) %>% sight
```
...
@MCCR:08. The `rddensity:rddensity` function performs an extension of this test. 
...
Each term is the mean difference of the outcome between the two periods for the two groups and the estimator is the difference of these differences. Consider as an example that the treatment is a job-training program implemented in 2021 and the output is wage observed in 2022. If the first term is 1000 $, this means that the average annual wage in the treatment group increased by 1000 $ in 2022 compared to 2021. This would be a relevant estimator of the effect of the program if nothing had changed on the labor market in 2022 compared to 2021. But if the economic situation improved, then the average wages will increase even for those who haven't been treated. This is measured by the second term in @eq-diff_diff (say 600 $). Therefore, the effect of the treatment is the difference between the two terms, which is 400 $.

As an example, @DITE:SCHA:04 sought to estimate the causal
...
interest is the number of car thefts. Denoting $D$ a dummy
equal to 1 if the block contains or is close to a Jewish institution
and $T$ a dummy equal to 1 after the attack, the treatment effect is,
...
two_obs %>% mutate(distance = ifelse(distance == "same", 1, 0)) %>% 
    lm(formula = thefts ~ period * distance) %>% sight
```

The effect of police on thefts is -0.07, which is very high as the average number of mountly theft per block is 0.09 and it is significant.
...
#| label: car_ttest
#| collapse: true
t.test(dt ~ distance, diffs) %>% sight
```
...
are not the same. This is the case in @HONG:13 article, who
...
which is performed on a quarterly basis and the data set is called `napster`. Napster was introduced in
...
group. From the `date` series, we construct a `period` variable using 1999, June as the cutoff:
...
```{r }
#| label: napster_lm
lm(expmusic ~ period * internet, napster, weight = weight) %>% sight
```

`internetyes` has a strong positive effect on expenditure. The interaction term between `period` and `internet` indicates that, after the deployment of Napster leads to a reduction of the expense for the "treated" individuals of 4.6 $, ie those who have internet access and that this effect is significant.
...
propensity score is the conditional probability (given *x*) of being
...
The matching estimator was first proposed by @ROSE:RUBI:83 and the interest in matching methods in econometrics realy started with   @DEHE:WAHB:99 and @DEHE:WAHB:02. These two articles revisited the results of @LALO:86 who showed that the estimation of treatment effect with observational data can be 
...
also proposed an algorithm, implemented in Stata by @BECK:ICHI:02:
...
where $\bar{y}_k ^ t$ and $\bar{y}_k ^ c$ are the mean values of the outcome in the treatment and in the control group  and $f_k = t_k / N_T$ is the frequency for strata $k$ for the treatment group (with $t_k$ is the number of treated individuals in strata $k$ and $N_T$ the total number of treated individuals). The
...
Sicilya. We restrict the sample to Tuscany:
...
the first one indicating the group and the second one
...
level, @BECK:ICHI:02 advice to use a low level with the following
...
- `strata` contains information about the strata (the frequencies, the
...
Finally, `step = "atet"` gives the values of the estimated ATET and different estimates of its standard deviation:
...
An alternative to using stratas and computing the ATET as a weighted average of the difference of the mean of the outcome between treated and control observations in each strata is to match each treated observation to one or several observations in
...
closest value of propensity score. Using the `model` tibble returned by `pscore`, we begin with constructing two tibbles, one for the treatment and one for the control group, containing only the index of the observation and the value of the propensity score:
...
and they are matched to respectively 319 and 297 observations in the control group. With **rolling** joins, one can
...
@ABAD:GARD:03 are refinement of comparative case studies, where
...
- the Legal Arizona Workers Act [@BOHN:LOFS:RAPH:14],
...
T_0+1, \ldots T$ are post-treatement periods. $y$ is the outcome and $x$ is a set of
...
where $w_n$ are unknown weights such that $w_n \geq 0$ and $\sum_{n=2} ^ {N+1}
= 1$. These weights define the synthetic
control for unit 1. 
...
solution of the minimization problem of @eq-seminorm:
...
@ABAD:21 discusses in length different choices for covariates' weights $v$.
The synthetic control method is implemented in **R** in two
packages: **Synth** [@ABAD:DIAM:HAIN:11] and **tidysynth**
[@DUNF:21]. We'll describe the use of the latter as it is written in the tidyverse style. 
